...
describe("quest lifecycle", () => {
	it("stops showing the Redbrook starter hint after the first quest is accepted", () => {
		...
		expect(
			zoneWelcomeText(starterZone, (questId) => sim.questState(questId)),
		).toBe("Find Marshal Redbrook in town — he has work for you.");

		const redbrook = [...sim.entities.values()].find(
...
describe("collision & terrain", () => {
	...
	it("NPCs spawn on dry land outside buildings", () => {
		...
		for (const e of sim.entities.values()) {
			...
			expect(
				groundHeight(e.pos.x, e.pos.z, SEED),
				`${e.name} underwater`,
			).toBeGreaterThan(WATER_LEVEL + 0.5);
			expect(
				isBlocked(SEED, e.pos.x, e.pos.z, 0.4),
	...
	it("mobs spawn out of deep water (murlocs may wade)", () => {
		...
		for (const e of sim.entities.values()) {
			...
			const canWade =
				MOBS[e.templateId].type === "murloc" || MOBS[e.templateId].canSwim;
			const min = canWade ? WATER_LEVEL - 0.55 : WATER_LEVEL + 0.35;
			expect(
				h,
				`${e.name} at ${e.pos.x.toFixed(0)},${e.pos.z.toFixed(0)}`,
			).toBeGreaterThan(min);
		}
	});
	...
	it("resolvePosition pushes points out of colliders", () => {
		...
		expect(Math.abs(inside.x - 10) + Math.abs(inside.z - 12)).toBeGreaterThan(
			0.5,
		);
	...
	it("camera ghosts through village buildings (hidden instead of pulling in)", () => {
		...
		expect(Math.abs(blocked.x - 10) + Math.abs(blocked.z - 12)).toBeGreaterThan(
			0.5,
		);
		...
		const overhead = cameraOcclusion(
			SEED,
			10,
			eyeY,
			4,
	...
	it("camera ghosts through campfires while movement still collides", () => {
		...
		expect(Math.abs(blocked.x - 3) + Math.abs(blocked.z + 4)).toBeGreaterThan(
			0.5,
		);
	});
	...
	it("camera ghosts through trees while movement still collides", () => {
		...
		expect(
			Math.abs(blocked.x - tree.x) + Math.abs(blocked.z - tree.z),
		).toBeGreaterThan(0.5);
	});
});
...
describe("swimming", () => {
	...
	it("ordinary mobs chase into deep water and keep dealing melee damage", () => {
		...
		expect(groundHeight(wolf.pos.x, wolf.pos.z, SEED)).toBeLessThan(
			WATER_LEVEL - 0.8,
		);
	...
	it("rare swimmers can chase into deep water", () => {
		...
		const rare = createMob(
			990001,
			MOBS.elder_bristleback,
			5,
			sim.groundPos(LAKE.x + 24, LAKE.z + 24),
		);
		for (let i = 0; i < 120; i++) {
			(sim as any).moveToward(
				rare,
				{ x: LAKE.x, y: 0, z: LAKE.z },
				rare.moveSpeed,
			);
		}
		expect(groundHeight(rare.pos.x, rare.pos.z, SEED)).toBeLessThan(
			WATER_LEVEL - 0.8,
		);
...
describe("rare spawn rules", () => {
	it("rare spawns are elite, control immune, swimmers with configured respawns", () => {
		for (const id of [
			...
			else if (id === "sableweb_matriarch")
				expect(MOBS[id].respawnMult).toBe(432);
			else if (id === "mirejaw_the_ravenous" || id === "sister_nhalia")
	...
	it("control auras do not stick to control-immune rares", () => {
		const sim = makeSim();
		const rare = createMob(990002, MOBS.sableweb_matriarch, 6, {
			x: 0,
			y: 0,
			z: 0,
		});
	...
	it("rare respawn timers use their configured multiplier", () => {
		...
		const rare = createMob(990003, MOBS.sableweb_matriarch, 6, {
			x: 0,
			y: 0,
			z: 0,
		});
...
describe("the Hollow Crypt doors", () => {
	it("walking into the door teleports you inside; walking into the exit brings you back", () => {
		...
		expect(
			dist2d(p.pos, { x: CRYPT_DOOR_POS.x, y: 0, z: CRYPT_DOOR_POS.z }) < 8,
		).toBe(true);
	});
	...
	it("places every dungeon entry and mob spawn on unblocked instance ground", () => {
		for (const dungeon of DUNGEON_LIST) {
			...
			if (dungeon.id === "nythraxis_boss_arena") {
				sim.players
					.get(sim.playerId)!
					.questsDone.add("q_nythraxis_bound_guardian");
				formRaid(sim);
			}
			...
			expect(
				p.pos.x,
				`${dungeon.id} entry is not inside an instance`,
			).toBeGreaterThan(DUNGEON_X_THRESHOLD);
			expect(
			...
			for (const mob of mobs) {
				expect(mob.hostile, `${dungeon.id} ${mob.name} is not hostile`).toBe(
					true,
				);
				expect(
			...
			for (const obj of objects) {
				expect(
					obj.lootable,
					`${dungeon.id} ${obj.name} is not interactable`,
				).toBe(true);
				expect(
...
describe("boss loot and encounter resets", () => {
	it("boss roll groups drop at most one item from each exclusive table", () => {
		...
		for (const [bossId, groupId, exactlyOne] of [
			...
			for (let i = 0; i < 300; i++) {
				...
				const dropped = (lootOf(mob)?.items ?? []).filter((s) =>
					groupItems.includes(s.itemId),
				);
				if (exactlyOne) {
					expect(
						dropped.length,
						`${bossId}/${groupId} kill #${i}`,
	...
	it("dungeon bosses always drop gear but cap bonus quality drops", () => {
		...
		for (const bossId of [
			...
			for (let i = 0; i < 300; i++) {
				...
				const uncommon = gear.filter(
					(s) => ITEMS[s.itemId]?.quality === "uncommon",
				);
	...
	it("awards need over greed regardless of the greed roll number", () => {
		...
		expect(
			sim.events.some(
				(e) => e.type === "loot" && e.text.includes("wins Greyjaw Hide Boots"),
			),
		).toBe(true);
	});
	...
	it("treats unanswered need-greed rolls as pass at timeout", () => {
		...
		expect(
			events.some(
				(e) =>
					e.type === "loot" &&
					e.text === "Everyone passed on Greyjaw Hide Boots.",
			),
		).toBe(true);
	});
	...
	it("returns all-passed need-greed loot to the corpse as open loot", () => {
		...
		expect(mob.loot?.items).toEqual([
			{ itemId: "greyjaw_hide_boots", count: 1, openToAll: true },
		]);
	...
	it("lets any player loot an all-passed need-greed item without starting another roll", () => {
		...
		expect(
			sim.events.some(
				(e) => e.type === "error" && e.text.includes("permission"),
			),
		).toBe(false);
		expect(sim.events.some((e) => e.type === "lootRoll")).toBe(false);
	});
	...
	it("quest drops stay on the corpse as personal loot for every eligible nearby party member", () => {
		...
		const boarHide = MOBS.wild_boar.loot.find(
			(entry) => entry.itemId === "boar_hide",
		)!;
	...
	it("does not drop a quest-gated item whose quest has no matching collect objective", () => {
		...
		const dropped = (mob.loot?.items ?? []).some(
			(slot) => slot.itemId === "greyjaw_fang",
		);
	...
	it("boss adds despawn on encounter reset instead of stacking across pulls", () => {
		...
		sim.tick();
		expect(p.pos.x).toBeGreaterThan(DUNGEON_X_THRESHOLD);
		const vael = [...sim.entities.values()].find(
			(e) => e.templateId === "vael_the_mistcaller",
		)!;
		const thralls = () =>
			[...sim.entities.values()].filter(
				(e) => e.templateId === "drowned_thrall",
			).length;
		...
		const aid = events.find(
			(e) => e.type === "log" && e.text.includes("calls for aid"),
		);
...
describe("quest npc roles", () => {
	it("every quest is listed in the questIds of its giver and turn-in NPCs", () => {
		...
		for (const quest of Object.values(QUESTS)) {
			expect(
				NPCS[quest.giverNpcId]?.questIds,
				`${quest.id} giver ${quest.giverNpcId}`,
			).toContain(quest.id);
			expect(
	...
	it("offers the Nythraxis attunement only from the Highwatch Aldric", () => {
		...
		expect(NPCS.brother_aldric.questIds).not.toContain(
			"q_nythraxis_restless_dead",
		);
		expect(NPCS.brother_aldric_fen.questIds).not.toContain(
			"q_nythraxis_restless_dead",
		);
		expect(NPCS.brother_aldric_highwatch.questIds).toContain(
			"q_nythraxis_restless_dead",
		);
	});
	...
	it("interacting with the turn-in NPC does not auto-accept an available quest", () => {
		...
		for (
			let i = 0;
			i < 10 && sim.questState("q_fenbridge_muster") !== "active";
			i++
		)
	...
	it("ends the Nythraxis attunement on the Bound Guardian quest", () => {
		...
		expect(NPCS.brother_aldric_highwatch.questIds).not.toContain(
	...
	it("gates the sealed crypt and grave visions behind Nythraxis quests", () => {
		...
		const bossArena = DUNGEON_LIST.find(
			(d) => d.id === "nythraxis_boss_arena",
		)!;
		...
		expect(logEvents).toContainEqual(
			expect.objectContaining({ entityId: vision?.id }),
		);
		expect(logEvents).toContainEqual(
			expect.objectContaining({ text: "My king was a good man." }),
		);
		let delayedEvents: SimEvent[] = [];
		for (let i = 0; i < 101; i++) delayedEvents = sim.tick();
		expect(delayedEvents).toContainEqual(
			expect.objectContaining({
				text: "I swore my blade to him.",
				entityId: vision?.id,
			}),
		);
		...
		expect([...sim.entities.values()].some((e) => e.id === vision!.id)).toBe(
			false,
		);
	});
	...
	it("shares Nythraxis grave progress and dialogue with nearby party members", () => {
		...
		expect(sim.questLog.get("q_nythraxis_graves")?.counts[0]).toBe(1);
		expect(
			sim.meta(allyPid)?.questLog.get("q_nythraxis_graves")?.counts[0],
		).toBe(1);
		const vision = [...sim.entities.values()].find(
			(e) => e.templateId === "vision_aldren_warrior",
		)!;
	...
	it("despawns Varkas Boneguards after 60 seconds out of combat without damage and resets on damage taken", () => {
		const sim = makeSim();
		const boneguard = createMob(909900, MOBS.varkas_boneguard, 19, {
			x: 0,
			y: 0,
			z: 0,
		});
	...
	it("shares Nythraxis ritual circle progress with nearby party members", () => {
		...
		expect(
			sim.meta(allyPid)?.questLog.get("q_nythraxis_bound_guardian")?.counts[0],
		).toBe(1);
	});
...
describe("pet heel warp", () => {
	it("keeps the spatial grid exact when a pet warps to its owner", () => {
		...
		const found: number[] = [];
		(sim as any).grid.forEachInRadius(p.pos.x, p.pos.z, 5, (e: Entity) =>
			found.push(e.id),
		);
...
describe("ranged auto-attack crit suppression", () => {
	...
	it("suppresses crit against a higher-level target, matching melee", () => {
		...
		const rolled = critChanceRolled(sim, () =>
...
describe("spell visuals", () => {
	it("hostile casts emit projectile spellfx events", () => {
		...
		expect(
			fx.some(
				(e) =>
					e.type === "spellfx" && e.fx === "projectile" && e.school === "fire",
			),
		).toBe(true);
	});
	...
	it("hostile targeted spells cannot start through dungeon walls", () => {
		...
		expect(
			events.some((e) => e.type === "castStart" && e.ability === "fireball"),
		).toBe(false);
		expect(
			events.some((e) => e.type === "error" && /line of sight/i.test(e.text)),
		).toBe(true);
	});
	...
	it("ranged auto shot does not fire through dungeon walls", () => {
		...
		expect(
			events.some((e) => e.type === "spellfx" && e.targetId === mob.id),
		).toBe(false);
		expect(
			events.some((e) => e.type === "damage" && e.ability === "Auto Shot"),
		).toBe(false);
	});
});

describe("mob auto attacks against moving targets", () => {
	function damageTimesFrom(
		events: SimEvent[],
		sourceId: number,
		targetId: number,
	): boolean {
