...
describe("threat from damage", () => {
	...
	it("defensive stance: -10% damage dealt, x1.3 threat on what lands", () => {
		...
		sim.tick();
		expect(sim.player.auras.some((a) => a.kind === "defensive_stance")).toBe(
			true,
		);
		const wolf = nearestMob(sim, "forest_wolf");
		beefUp(wolf);
		...
		expect(wolf.threat.get(sim.playerId)).toBeCloseTo(
			90 * DEFENSIVE_STANCE_THREAT_MULT + 1,
			5,
		);
		// stance is a toggle
	...
	it("righteous fury multiplies HOLY threat by 1.6 and leaves physical alone", () => {
		...
		expect(wolf.threat.get(sim.playerId)).toBeCloseTo(
			100 * RIGHTEOUS_FURY_THREAT_MULT + 1,
			5,
		);
		hit(sim, sim.player, wolf, 100, "physical");
		expect(wolf.threat.get(sim.playerId)).toBeCloseTo(
			100 * RIGHTEOUS_FURY_THREAT_MULT + 101,
			5,
		);
	});
	...
	it("consecration burns the ground every 2 seconds from 0s to 8s and generates holy threat each pulse", () => {
		...
		expect(
			damageEvents.reduce((sum, amount) => sum + amount, 0),
		).toBeGreaterThanOrEqual(28 * 5);
		expect(wolf.threat.get(sim.playerId) ?? 0).toBeGreaterThan(
			28 * 5 * RIGHTEOUS_FURY_THREAT_MULT,
		);
	});
...
describe("healing threat", () => {
	function partyOfTwo() {
		...
		return {
			sim,
			tank: sim.entities.get(tank)!,
			healer: sim.entities.get(healer)!,
		};
	}
...
describe("taunt and growl", () => {
	...
	it("growl requires bear form", () => {
		...
		sim.player.facing = Math.atan2(
			wolf.pos.x - sim.player.pos.x,
			wolf.pos.z - sim.player.pos.z,
		);
...
describe("rogue stealth", () => {
	...
	it("cannot stealth in combat; acting breaks stealth; ambush requires it", () => {
		...
		expect(
			events.some((e) => e.type === "error" && /stealthed/.test(e.text)),
		).toBe(true);
	...
	it("sprint can be used before or during stealth without breaking stealth", () => {
		...
		sim.castAbility("stealth");
		expect(
			sim.player.auras.some((a) => a.id === "stealth" && a.kind === "stealth"),
		).toBe(true);
		...
		expect(
			sim.player.auras.some(
				(a) => a.id === "sprint" && a.kind === "buff_speed",
			),
		).toBe(true);
...
describe("hunter pets", () => {
	...
	it("friendly target spells can affect controlled pets", () => {
		...
		expect((sim as any).effectiveAttackPower(pet)).toBeGreaterThan(
			attackPowerBefore,
		);
	...
	it("keeps the owner in combat while their pet tanks a mob", () => {
		...
		for (let i = 0; i < 20 * 20 && boar.aggroTargetId !== pet.id; i++)
			sim.tick();
		expect(boar.aggroTargetId).toBe(pet.id);
	...
	it("dismiss does not release permanent pets back to the wild", () => {
		...
		expect(
			sim
				.tick()
				.some((e) => e.type === "error" && /Permanent pets/.test(e.text)),
		).toBe(true);
	...
	it("a tamed beast that dies stays owned until revived or abandoned", () => {
		...
		sim.player.facing = Math.atan2(
			boar.pos.x - sim.player.pos.x,
			boar.pos.z - sim.player.pos.z,
		);
		sim.castAbility("tame_beast");
		const events = sim.tick();
		expect(
			events.some(
				(e) => e.type === "error" && /already have a pet/.test(e.text),
			),
		).toBe(true);
	...
	it("pet name and dead state persist through character serialization", () => {
		...
		restored.entities.get(pid)!.resource =
			restored.entities.get(pid)!.maxResource;
	...
	it("pet taunt is commandable and uses a 10 second cooldown", () => {
		...
		for (let i = 0; i < 20 * 2 && boar.forcedTargetId !== pet.id; i++)
			sim.tick();
		...
		sim.petTaunt();
		expect(
			sim.tick().some((e) => e.type === "error" && /not ready/.test(e.text)),
		).toBe(true);
	...
	it("tame validation: too-high level and elites are refused", () => {
		...
		const events = sim.tick();
		expect(
			events.some((e) => e.type === "error" && /too high level/.test(e.text)),
		).toBe(true);
...
describe("druid forms", () => {
	...
	it("bear form raises armor, maximum health, and attack power", () => {
		...
		sim.tick();
		expect(sim.player.auras.some((a) => a.kind === "form_bear")).toBe(true);
		expect(sim.player.stats.armor).toBeGreaterThanOrEqual(
			Math.round(armorBefore * 1.9),
		);
	...
	it("bear charge is learned with Bear Form and only works while shifted", () => {
		...
		sim.setPlayerLevel(10);
		expect(
			abilitiesKnownAt("druid", 10).some((a) => a.def.id === "bear_charge"),
		).toBe(true);
		const wolf = nearestMob(sim, "forest_wolf");
		beefUp(wolf);
		...
		expect(
			sim.tick().some((e) => e.type === "error" && /Bear Form/.test(e.text)),
		).toBe(true);
	...
	it("claw needs wolf form, builds combo points, and ferocious bite spends them", () => {
		...
		const events = sim.tick();
		expect(
			events.some((e) => e.type === "error" && /Wolf Form/.test(e.text)),
		).toBe(true);
		sim.castAbility("cat_form");
		sim.tick();
		let guard = 0;
	...
	it("caster spells are locked while shapeshifted", () => {
		...
		const events = sim.tick();
		expect(
			events.some((e) => e.type === "error" && /shapeshifted/.test(e.text)),
		).toBe(true);
	});
	...
	it("bear and wolf forms can only use their own form kits", () => {
		...
		events = sim.tick();
		expect(
			events.some((e) => e.type === "error" && /Bear Form/.test(e.text)),
		).toBe(true);
	...
	it("bear form learns demoralizing roar at level 10 and lowers nearby mob attack power", () => {
		...
		expect((sim as any).effectiveAttackPower(wolf)).toBe(
			Math.max(0, apBefore - 20),
		);
	...
	it("wolf form gains agility/AP and supports prowl into rake bleed opener", () => {
		...
		sim.tick();
		expect(sim.player.auras.some((a) => a.kind === "stealth")).toBe(false);
		expect(wolf.auras.some((a) => a.id === "rake" && a.kind === "dot")).toBe(
			true,
		);
		expect(sim.player.comboPoints).toBeGreaterThanOrEqual(1);
	});
	...
	it("prowl slows movement without rooting and toggles off", () => {
		...
		sim.tick();
		expect(
			sim.player.auras.some((a) => a.id === "prowl" && a.kind === "stealth"),
		).toBe(true);
...
describe("on-next-swing cooldowns (#56)", () => {
	it("Raptor Strike applies its 6s cooldown when the queued swing resolves", () => {
		...
		for (let i = 0; i < 20 * 4 && sim.player.queuedOnSwing !== null; i++)
			sim.tick();
...
describe("shaman travel and shock mechanics", () => {
	...
	it("Ghost Wolf toggles speed and survives damage events", () => {
		...
		expect(
			sim.player.auras.some(
				(a) => a.id === "ghost_wolf" && a.kind === "buff_speed",
			),
		).toBe(true);
	...
	it("Ghost Wolf stays active through Lightning Shield contact, jump, and respawn cleanup", () => {
		...
		expect(sim.player.auras.some((a) => a.id === "lightning_shield")).toBe(
			true,
		);
		for (let i = 0; i < 40; i++) sim.tick();
		...
		wolf.facing = Math.atan2(
			sim.player.pos.x - wolf.pos.x,
			sim.player.pos.z - wolf.pos.z,
		);
...
describe("warlock demon summons", () => {
	...
	it("warlocks heal demons with mana instead of food and cannot abandon them", () => {
		...
		sim.feedPet("baked_bread");
		expect(
			sim.tick().some((e) => e.type === "error" && /Only hunters/.test(e.text)),
		).toBe(true);
		...
		expect(
			sim.events.some(
				(e) => e.type === "castStart" && (e as any).ability === "demon_heal",
			),
		).toBe(true);
