...
describe("i18n Localization Key Coverage", () => {
	...
	beforeAll(async () => {
		await Promise.all(
			supportedLanguages.map((lang) => ensureLocaleLoaded(lang)),
		);
	});

	const placeholderPattern =
		/\b(TODO|TBD|FIXME|PLACEHOLDER|TRANSLATE|LOREM)\b/i;
	...
	function verifyKeys(
		...
		for (const key in base) {
			...
			if (typeof baseValue === "object" && baseValue !== null) {
				...
				expect(
					text,
					`${currentPath} should not contain placeholder markers`,
	...
	function flattenStrings(
		...
		for (const [key, value] of Object.entries(base)) {
			...
			if (typeof value === "string") {
				...
				entries.push(
					...flattenStrings(value as Record<string, unknown>, currentPath),
				);
			}
		}
		return entries;
	...
	type EntityManifestEntry = ReturnType<
		typeof entityTranslationManifest
	>[number];
	...
	function parseIndexedEntry(
		...
		if (!Number.isInteger(index))
			throw new Error(`Malformed indexed entity index: ${id}`);
		return { ownerId, index };
	}

	function worldRequest(entry: EntityManifestEntry): EntityRequest {
		if (entry.kind === "mob")
			return { kind: "mob", id: entry.id, field: "name" };
		if (entry.kind === "npc") {
			return {
				kind: "npc",
				id: entry.id,
		...
		if (entry.kind === "zone") {
			return {
				kind: "zone",
				id: entry.id,
				field: entry.field as "name" | "welcome",
			};
		}
	...
	function sourceFilesUnder(relativeDir: string): string[] {
		...
		for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
			...
			if (entry.isDirectory())
				files.push(
					...sourceFilesUnder(path.relative(process.cwd(), entryPath)),
				);
	...
	it("should resolve nested keys accurately using t() helper", () => {
		...
		expect(t("loading.worldProgress", { done: 3, total: 9 })).toBe(
			"Loading world... 3/9",
		);
		...
		expect(t("character.levelClass", { level: 7, className: "Maga" })).toBe(
			"Nivel 7 Maga",
		);
	...
	it("should expose typed locale utilities for shell metadata and formatting", () => {
		...
		expect(formatNumber(1234.5, { maximumFractionDigits: 1 }, "de_DE")).toBe(
			"1.234,5",
		);
		expect(
			formatDateTime(
	...
	it("should enumerate entity source coverage for later translation work", () => {
		...
		expect(new Set(manifest.map((entry) => entry.key)).size).toBe(
			manifest.length,
		);
		...
		expect(entityCount("dungeon", "enterText")).toBe(
			Object.keys(DUNGEONS).length,
		);
		expect(entityCount("dungeon", "leaveText")).toBe(
			Object.keys(DUNGEONS).length,
		);
	});
	...
	it("should provide every class and ability translation in every locale", () => {
		...
		expect(missingEntityTranslationsForGroups(["classAbility"])).toHaveLength(
			0,
		);
		...
		for (const lang of supportedLanguages) {
			...
			for (const entry of classAbilityEntries) {
				...
				expect(rendered.trim().length, `${lang}.${entry.key}`).toBeGreaterThan(
					0,
				);
	...
	it("should provide every item translation in every locale without canonical fallbacks", () => {
		const itemEntries = entityTranslationManifest().filter(
			(entry) => entry.group === "item",
		);
		expect(itemEntries).toHaveLength(Object.keys(ITEMS).length);
		expect(
			missingEntityTranslationsForGroups(["classAbility", "item"]),
		).toHaveLength(0);
		...
		expect(
			tEntity({ kind: "item", id: "gravecaller_sigil", field: "name" }),
		).toBe("Gravecallers Siegel");
		expect(entityTranslationFallbackLog()).toHaveLength(0);
	...
	it("should route class-detail damage ranges through localized templates", () => {
		...
		expect(source).not.toContain(
			" plus ${primaryEffect.perCombo} per combo point",
		);

		setLanguage("de_DE");
		expect(t("abilityUi.tooltip.damageRange", { min: "16", max: "25" })).toBe(
			"16 bis 25",
		);
		setLanguage("zh_CN");
		expect(t("abilityUi.tooltip.damageRange", { min: "16", max: "25" })).toBe(
	...
	it("should expose no missing entity translations across all entity groups", () => {
		const classAbilityMissing = missingEntityTranslationsForGroups([
			"classAbility",
		]);
		...
		expect(
			missingEntityTranslationsForGroups(["classAbility", "item", "world"]),
		).toHaveLength(0);
		expect(() => assertEntityTranslationsReady([])).not.toThrow();
		expect(() => assertEntityTranslationsReady(["classAbility"])).not.toThrow();
		expect(() =>
			assertEntityTranslationsReady(["classAbility", "item"]),
		).not.toThrow();
		expect(() =>
			assertEntityTranslationsReady(["classAbility", "item", "world"]),
		).not.toThrow();
	});

	it("should provide every world-content translation in every locale without canonical fallbacks", () => {
		const worldEntries = entityTranslationManifest().filter(
		...
		setLanguage("de_DE");
		expect(tEntity({ kind: "mob", id: "forest_wolf", field: "name" })).toBe(
			"Waldwolf",
		);
		expect(tEntity({ kind: "quest", id: "q_wolves", field: "title" })).toBe(
			"Wölfe vor der Tür",
		);
		...
		setLanguage("zh_CN");
		expect(
			tEntity({ kind: "quest", id: "q_gravewyrm", field: "title" }),
		).toContain("科祖尔");

		setLanguage("ja_JP");
		expect(
			tEntity({ kind: "dungeon", id: "hollow_crypt", field: "name" }),
		).toBe("虚ろの墓所");
	...
	it("should provide talent content translations for every supported locale", () => {
		...
		for (const lang of supportedLanguages) {
			...
			for (const entry of talentEntries) {
				...
				expect(
					...
					`${lang}.${entry.id}.${entry.field}`,
				).toBeGreaterThan(0);
				expect(rendered, `${lang}.${entry.id}.${entry.field}`).not.toMatch(
					placeholderPattern,
				);
		...
		if (RELEASE_TIER) {
			...
			expect(
				renderTalentManifestEntry(
					talentEntries.find(
						(entry) => entry.id === "war_toughness" && entry.field === "name",
					)!,
				),
			).toContain("Dureza");
			expect(
				renderTalentManifestEntry(
					talentEntries.find(
						(entry) =>
							entry.id === "arms.mastery" && entry.field === "description",
					)!,
				),
			).toContain("daño");
			...
			expect(
				renderTalentManifestEntry(
					talentEntries.find(
						(entry) => entry.id === "war_cruelty" && entry.field === "name",
					)!,
				),
			).toContain("残忍");
			...
			expect(
				renderTalentManifestEntry(
					talentEntries.find(
						(entry) =>
							entry.id === "prot_choice.pc_last_stand" &&
							entry.field === "description",
					)!,
				),
			).toContain("생명력");
			...
			for (const lang of checkedLanguages) {
				...
				for (const questId of questIds) {
					...
					for (const pattern of genericPatterns) {
						expect(
							text,
							`${lang}.${questId}.text generic narrative`,
						).not.toMatch(pattern);
						expect(
							completion,
							`${lang}.${questId}.completion generic narrative`,
						).not.toMatch(pattern);
					}
				...
				expect(
					textSkeletons.size,
					`${lang} quest text skeleton diversity`,
				).toBeGreaterThan(Math.floor(questIds.length * 0.8));
				expect(
	...
	it.runIf(RELEASE_TIER)(
		...
		() => {
			const nonLatin: Record<string, typeof en> = {
				zh_CN,
				zh_TW,
				ja_JP,
				ko_KR,
			...
			for (const [lang, data] of Object.entries(nonLatin)) {
				const quests =
					(data as { entities?: { quests?: unknown } }).entities?.quests ?? {};
	...
	it("should keep Traditional Chinese world content out of Simplified-only shortcuts", () => {
		...
		for (const entry of worldEntries) {
			const rendered = tEntity(worldRequest(entry));
			expect(rendered, `zh_TW.${entry.key}`).not.toMatch(
				simplifiedOnlyCharacters,
			);
		}

		expect(
			t("worldContent.dungeonInstanceBusy", { name: "墓龍聖所" }),
		).toContain("佔用");
		expect(
			t("worldContent.dungeonInstanceBusy", { name: "墓龍聖所" }),
		).not.toMatch(simplifiedOnlyCharacters);
		setLanguage("en");
	});

	it("should keep the entity resolver out of simulation and server modules", () => {
		for (const file of [
			...sourceFilesUnder("src/sim"),
			...sourceFilesUnder("server"),
		]) {
			const source = fs.readFileSync(file, "utf8");
			expect(source, file).not.toMatch(
				/(?:from|import)\s+["'][^"']*ui\/(?:i18n|entity_i18n)["']/,
			);
		}
	});
	...
	it("should route rendered world-content labels through localized entity helpers", () => {
		const hudSource = fs.readFileSync(
			path.resolve(process.cwd(), "src/ui/hud.ts"),
			"utf8",
		);
		expect(hudSource).toContain("zoneDisplayName");
		expect(hudSource).toMatch(
			/\$\(["']#zone-label["']\)\.textContent = zoneDisplayName/,
		);
	...
	it("should preserve and render every HUD interpolation placeholder in every locale", () => {
		...
		for (const { key, expected } of hudDynamicKeys) {
			for (const [lang, locale] of Object.entries(allLocales)) {
				...
				expect(placeholders(template), `${lang}.${key} placeholders`).toEqual(
					expected,
				);
				...
				expect(
					rendered,
					`${lang}.${key} should not leave placeholders unresolved`,
				).not.toMatch(/\{[A-Za-z][A-Za-z0-9]*\}/);
				for (const placeholder of expected) {
					expect(
						rendered,
						`${lang}.${key} should include ${placeholder}`,
					).toContain(String(interpolationValues[placeholder]));
				}
			}
		}
	...
	it("should interpolate combat, chat, and log templates without dropping values", () => {
		...
		setLanguage("ja_JP");
		const guildChat = t("hud.chat.templates.guild", {
			name: "Aki",
			message: "集合",
		});
		...
		setLanguage("zh_CN");
		expect(t("hud.logs.lootReceiveItem", { item: "粗糙护腕" })).toContain(
			"粗糙护腕",
		);
	...
	it("should format ability tooltip templates without dropping dynamic values", () => {
		setLanguage("de_DE");
		expect(t("abilityUi.tooltip.cooldownSeconds", { seconds: 8 })).toContain(
			"8",
		);
		expect(t("abilityUi.spellbook.trainableAtLevel", { level: 10 })).toContain(
			"10",
		);
		...
		setLanguage("ja_JP");
		const finisher = t("abilityUi.tooltip.finisherDamage", {
			base: 14,
			perCombo: 7,
		});
	...
	it("should format quest UI templates without dropping dynamic values", () => {
		setLanguage("de_DE");
		expect(t("questUi.log.summary", { active: 3, completed: 8 })).toContain(
			"3",
		);
		expect(t("questUi.log.summary", { active: 3, completed: 8 })).toContain(
			"8",
		...
		setLanguage("fr_FR");
		expect(
			t("questUi.dialog.availableQuestAria", { name: "A Swift Response" }),
		).toContain("A Swift Response");
	...
	it("should format item UI and money helpers without dropping dynamic values", () => {
		setLanguage("de_DE");
		expect(t("itemUi.vendor.goodsTitle", { name: "Haldren" })).toContain(
			"Haldren",
		);
		expect(t("itemUi.market.sellNote", { cut: 5, used: 2, max: 12 })).toContain(
			"5",
		);
		...
		setLanguage("ja_JP");
		expect(t("itemUi.tooltip.useFood", { amount: 61, seconds: 18 })).toContain(
			"61",
		);
	...
	it("should expose all supported hreflang alternates in index.html", () => {
		...
		for (const hreflang of expectedHreflang) {
			expect(html, `missing hreflang ${hreflang}`).toContain(
				`hreflang="${hreflang}"`,
			);
		}
