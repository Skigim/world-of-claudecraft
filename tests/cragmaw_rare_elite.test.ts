...
describe("Old Cragmaw — rare elite ridge beast (Thornpeak Heights)", () => {
	...
	it("carries only existing, composable mechanics (a rending pounce + wounded enrage)", () => {
		const m = MOBS.old_cragmaw;
		expect(m.aoePulse).toMatchObject({
			name: "Savage Pounce",
			school: "physical",
		});
		expect(m.enrage).toMatchObject({
			belowHpPct: 0.35,
			dmgMult: 1.4,
			hasteMult: 1.3,
		});
	...
	it("drops its unique rare boots and every loot itemId resolves", () => {
		...
		expect(boots.stats!.armor!).toBeGreaterThan(
			ITEMS.ridgestalker_treads.stats!.armor!,
		);
	});
	...
	it("spawns into a live sim with elite-scaled health above a normal Ridge Stalker", () => {
		...
		const cragmaw = createMob((sim as any).nextId++, MOBS.old_cragmaw, 14, {
			x: 0,
			y: 0,
			z: 560,
		});
		const stalker = createMob((sim as any).nextId++, MOBS.ridge_stalker, 14, {
			x: 4,
			y: 0,
			z: 560,
		});
