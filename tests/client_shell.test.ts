...
describe("client HTML shell", () => {
	...
	it("keeps the Account nav tab hidden unless a session is restored", () => {
		expect(html).toContain(
			'<li class="nav-item" id="nav-item-account" hidden>',
		);
	...
	it("shows a logged-in Logout nav item next to Account", () => {
		...
		expect(html.indexOf('id="nav-btn-account"')).toBeLessThan(
			html.indexOf('id="nav-btn-logout"'),
		);
	...
	it("ships crawlable SEO metadata and sitemap hints", () => {
		...
		expect(html).toContain(
			'<link rel="canonical" href="https://worldofclaudecraft.com/" />',
		);
		expect(html).toContain(
		...
		expect(sitemapXml).toContain(
			"<loc>https://worldofclaudecraft.com/data-deletion</loc>",
		);
		expect(sitemapXml).toContain(
			"<loc>https://worldofclaudecraft.com/support</loc>",
		);
		...
		expect(supportHtml).toContain(
			'href="/data-deletion">Data Deletion page</a>',
		);
		expect(supportHtml).toContain('"@type": "ContactPage"');
		expect(html).toContain(
			'href="/terms" class="footer-link" data-i18n="footer.terms"',
		);
		expect(html).toContain(
			'href="/privacy" class="footer-link" data-i18n="footer.privacy"',
		);
	...
	it("loads Meta Pixel outside local development and tracks level 5", () => {
		...
		expect(hudTs).toMatch(
			/fbq\(["']trackCustom["'],\s*eventName,\s*data \?\? \{\}\);/,
		);
		expect(hudTs).toMatch(
			/if \(ev\.level === 5\)\s*trackMetaPixel\(["']ReachedLevel5["'], \{ level: ev\.level \}\);/,
		);
	});
	...
	it("excludes wallet verification surfaces from native app builds", () => {
		...
		expect(html).toContain(
			'<section class="account-card account-wallet-card">',
		);
		expect(mainTs).toContain(
	...
	it("keeps the game menu free of duplicate and dev-only entries", () => {
		const interfaceEntries =
			hudTs.match(
				/add\(t\(["']hud\.options\.interface["']\), \(\) => goto\(["']interface["']\)\);/g,
			) ?? [];
	...
	it("collapses in-game mobile community links behind one Community control", () => {
		...
		expect(html).toContain(
			"body.mobile-touch .community-link.donate { display: inline-flex;",
		);
		expect(html).not.toContain(
			"body.mobile-touch .community-link.donate {\n    display: none;",
		);
		expect(html).not.toContain(
			"body.mobile-touch .donate-cta {\n    display: none;",
		);
	});

	it("closes mobile community and More trays when tapping outside", () => {
		expect(hudTs).toMatch(
			/const communityMenu = document\.getElementById\(\s*["']community-menu["'],?\s*\) as HTMLDetailsElement \| null;/,
		);
		expect(hudTs).toMatch(
		...
		expect(hudTs).not.toMatch(
			/if \(communityMenu\?\.open && !communityMenu\.contains\(target\)\) \{\s*communityMenu\.open = false;\s*\}/,
		);
		...
		expect(hudTs).toMatch(
			/document\s*\.getElementById\(["']mobile-controls["']\)\s*\?\.classList\.remove\(["']expanded["']\);/,
		);
		expect(hudTs).toContain('more?.classList.remove("active");');
		expect(hudTs).toMatch(
			/document\s*\.getElementById\(["']mobile-more-close["']\)\s*\?\.addEventListener\(["']click["'], \(\) => \{/,
		);
	});
	...
	it("lays out mobile More tray buttons horizontally", () => {
		...
		expect(
			bindButton.indexOf("button.closest('#mobile-extra-controls')"),
		).toBeGreaterThan(-1);
		expect(bindButton.indexOf("this.closeMoreModal();")).toBeLessThan(
	...
	it("keeps the World Market to one scroll container with browse filters below the tabs", () => {
		...
		expect(html).toContain(
			"#market-body { overflow-y: auto; flex: 1; min-height: 0;",
		);
		expect(html).toContain(
		...
		expect(hudTs).toMatch(
			/class="mkt-filters\$\{hasSubtype \? ["'] has-subtype["'] : ["']["']\}"/,
		);
	...
	it("keeps the mobile More and Autorun buttons in the combat row", () => {
		...
		const primaryButtons = [
			...combatControls.matchAll(/<button class="mobile-btn"/g),
		];
		...
		expect(html).toContain(
			"pointer-events: auto; align-items: end; z-index: 30;",
		);
		expect(html).toContain(
	...
	it("seeds druid form bars with the form kit, and only clones normal for rogue stealth", () => {
		...
		expect(hudTs).toMatch(
			/const emptyFormMap =\s*this\.activeHotbarForm !== ["']normal["'] &&\s*parsed\.every\(\(action\) => action === null\);/,
		);
		expect(hudTs).toMatch(
			/localStorage\.getItem\(this\.slotMapKey\(["']normal["']\)\)/,
		);
		expect(hudTs).not.toMatch(
	...
	it("only auto-places abilities that belong on the active form bar", () => {
		expect(hudTs).toMatch(
			/if \(this\.shouldAutoPlaceOnForm\(id, this\.activeHotbarForm\)\)\s*autoPlaceAbilityIds\.add\(id\);/,
		);
	});

	it("keeps the active druid form toggle on its form action bar", () => {
		expect(hudTs).toMatch(
			/new Set\(\[\s*["']bear_form["'],\s*["']cat_form["'],\s*["']travel_form["'],?\s*\]\)/,
		);
		expect(hudTs).toMatch(
		...
		expect(hudTs).toMatch(
			/if \(formToggle && knownAbilityIds\.includes\(formToggle\)\)\s*autoPlaceAbilityIds\.add\(formToggle\);/,
		);
	});
	...
	it("offers a reset-to-default action bar button in the spellbook, only for classes with form bars", () => {
		...
		expect(hudTs).toMatch(/t\(["']abilityUi\.spellbook\.resetBar["']\)/);
	...
	it("shows mobile spellbook add and remove controls for the spell bar", () => {
		...
		expect(hudTs).toMatch(
			/toggle\.className =\s*["']spell-hotbar-toggle["'] \+ \(onBar \? ["'] remove["'] : ["']["']\);/,
		);
	...
	it("combines Trader and Bags into a mobile split-pane modal", () => {
		...
		expect(hudTs).toMatch(
			/if \(this\.vendorOpen && document\.body\.classList\.contains\(["']mobile-touch["']\)\)\s*this\.closeVendor\(\);/,
		);
		expect(hudTs).toMatch(
