...
describe("Sloomtooth the Drowned (rare elite)", () => {
	...
	it("references only loot items that actually exist", () => {
		for (const l of MOBS.sloomtooth_the_drowned.loot) {
			if (l.itemId)
				expect(ITEMS[l.itemId], `missing item ${l.itemId}`).toBeDefined();
		}
	});
	...
	it("actually surges back to life the first time it is brought low", () => {
		...
		const mob = createMob(990201, MOBS.sloomtooth_the_drowned, 11, {
			x: 0,
			y: 0,
			z: 0,
		});
		...
		const before = mob.hp;
		(
			sim as unknown as { updateBossMechanics(m: typeof mob): void }
