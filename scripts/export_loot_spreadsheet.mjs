...
const NEW_LOOT_ADDED = [
	...
	{
		itemId: "sableweb_slippers",
		section: "Open World Rare Spawn",
		subgroup: "Eastbrook Vale - Sableweb Matriarch",
	},
	{
...
function addSource(itemId, source) {
	...
	if (!item) {
		sourceRows.push({
			...
			gated_by_quest_name: source.gated_by_quest_id
				? questName(source.gated_by_quest_id)
				: "",
...
for (const mob of Object.values(MOBS)) {
	for (const entry of mob.loot ?? []) {
		...
		addSource(entry.itemId, {
			...
			source_detail: `${mob.type}, level ${mob.minLevel}-${mob.maxLevel}${mob.boss ? ", boss" : ""}${mob.elite ? ", elite" : ""}${mob.rare ? ", rare" : ""}`,
...
for (const quest of Object.values(QUESTS)) {
	...
	for (const cls of ALL_CLASSES) {
		const rewardItem =
			quest.itemRewards?.[cls] ?? quest.itemRewards?.[REWARD_ARCHETYPE[cls]];
...
const newLootRows = NEW_LOOT_ADDED.flatMap((meta) => {
	...
	const rows = (sourcesByItem.get(meta.itemId) ?? []).filter(
		(row) =>
			row.source_type === "mob_drop" &&
			(!meta.sourceIds || meta.sourceIds.includes(row.source_id)),
	);
