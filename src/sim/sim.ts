...
const DAMAGE_IDLE_DESPAWN_MOB_IDS = new Set([
	"varkas_boneguard",
	"bound_guardian",
]);
...
const SHAMAN_SHOCK_COOLDOWN_IDS = [
	"earth_shock",
	"flame_shock",
	"frost_shock",
] as const;
...
const EMOTES: Record<string, EmoteDef> = {
	...
	greet: {
		solo: "greets everyone with a hearty hello.",
		target: "greets %t with a hearty hello.",
	},
	...
	cower: {
		solo: "cowers in fear.",
		target: "cowers in fear at the sight of %t.",
	},
};
...
export function computeQuestState(
	...
	if (quest.requiresQuest && !questsDone.has(quest.requiresQuest))
		return "unavailable";
...
function isStealthToggle(ability: AbilityDef): boolean {
	return ability.effects.some(
		(e) => e.type === "selfBuff" && e.kind === "stealth",
	);
}
...
export class Sim {
	...
	private addEntity(e: Entity): void {
		...
		if (e.templateId === "dungeon_door" && this.dungeonDoorIds)
			this.dungeonDoorIds.push(e.id);
	}
	...
	addPlayer(
		...
		const savedArena1v1: ArenaStanding = {
			rating:
				opts?.state?.arena1v1Rating ??
				opts?.state?.arenaRating ??
				ARENA_BASE_RATING,
		...
		if (opts?.state) {
			...
			if (typeof s.activeLoadout === "number")
				meta.activeLoadout = s.activeLoadout;
			if (s.raidLockouts) {
	...
	unequipMechChroma(chromaId: string, pid?: number): boolean {
		...
		this.accountCosmetics = {
			...this.accountCosmetics,
			mechChromaIds: this.accountCosmetics.mechChromaIds.filter(
				(id) => id !== chromaId,
			),
		};
	...
	private talentLockReason(p: Entity): string | null {
		...
		if (this.arenaMatches.has(p.id))
			return "You cannot change talents during an arena match.";
		return null;
	}
	...
	talentPoints(pid?: number): { total: number; spent: number } {
		...
		return {
			total: talentPointsAtLevel(r.e.level),
			spent: pointsSpent(r.meta.talents),
		};
	}
	...
	applyTalents(alloc: TalentAllocation, pid?: number): boolean {
		...
		if (sanitized.spec && r.e.level < FIRST_TALENT_LEVEL) {
			this.error(
				r.e.id,
				`You may choose a specialization at level ${FIRST_TALENT_LEVEL}.`,
			);
			return false;
		}
	...
	saveLoadout(
		...
		const lo: SavedLoadout = {
			name: clean,
			alloc: cloneAllocation(r.meta.talents),
			bar: safeBar,
		};
	...
	deleteLoadout(index: number, pid?: number): boolean {
		...
		if (wasActive) {
			r.meta.activeLoadout =
				r.meta.loadouts.length > 0
					? Math.min(index, r.meta.loadouts.length - 1)
					: -1;
			const next =
				r.meta.activeLoadout >= 0
					? r.meta.loadouts[r.meta.activeLoadout]
					: null;
			if (next) {
	...
	resolvedAbility(abilityId: string, pid?: number): ResolvedAbility | null {
		...
		if (tax > 1 && found.cost > 0)
			return { ...found, cost: Math.ceil(found.cost * tax) };
		return found;
	}
	...
	tick(): SimEvent[] {
		...
		for (const e of this.entities.values()) {
			...
			if (
				...
				e.damageIdleDespawnTimer =
					(e.damageIdleDespawnTimer ?? DAMAGE_IDLE_DESPAWN_SECONDS) - DT;
		...
		for (const e of this.entities.values()) {
			...
			if (
				e.ownerId !== null &&
				e.aggroTargetId !== null &&
				e.combatTimer < PET_COMBAT_LINGER
			)
	...
	private blindMissBonus(e: Entity): number {
		let bonus = 0;
		for (const a of e.auras)
			if (a.kind === "blind" && a.value > bonus) bonus = a.value;
		return bonus;
	}
	...
	private mobCanSwim(
		template: { type?: string; canSwim?: boolean } | undefined,
	): boolean {
		return !!template;
	}
	private mobCanSpawnInWater(
		...
		return (
			!!template && (template.canSwim === true || template.type === "murloc")
		);
	}
	...
	private isNythraxisRaidEnemy(target: Entity): boolean {
		return (
			target.kind === "mob" &&
			(target.templateId === NYTHRAXIS_BOSS_ID ||
	...
	private partyLootCandidatesForMob(mob: Entity): PlayerMeta[] {
		...
		for (const pid of party.members) {
			...
			const e = this.entities.get(pid);
			if (candidate && e && !e.dead && dist2d(e.pos, mob.pos) <= PARTY_XP_RANGE)
	...
	private effectiveItemLootStrategy(
		itemId: string,
		mob: Entity,
	): ItemLootStrategy {
		...
		return q === "poor" || q === "common"
			? strategies.commonItems
			: strategies.premiumItems;
	}
	private moveSpeedMult(e: Entity): number {
		let slow = 1,
			speed = 1;
		for (const a of e.auras) {
			if (a.kind === "slow" || a.kind === "stealth")
				slow = Math.min(slow, a.value);
			// buff_speed and form_travel both carry a 1+fraction multiplier (1.4 = +40%).
			if (a.kind === "buff_speed" || a.kind === "form_travel")
	...
	private fleeMoveSpeed(e: Entity): number {
		return new MobRuntime(e, MOBS[e.templateId]).fleeMoveSpeed(
			this.moveSpeedMult(e),
		);
	}

	private recoverFromFlee(
		mob: Entity,
		target: Entity,
		leash: number,
		leashAnchor: Vec3,
		...
		mob.aiState =
			dist2d(mob.pos, target.pos) > MELEE_RANGE ? "chase" : "attack";
		mob.fleeTimer = 0;
		if (
			new MobRuntime(mob, MOBS[mob.templateId]).canDistanceLeash(
				DUNGEON_X_THRESHOLD,
			) &&
	...
	private jumpMult(e: Entity): number {
		let m = 1;
		for (const a of e.auras)
			if (a.kind === "buff_jump") m = Math.max(m, a.value);
		return m;
	}
	...
	isSwimming(e: Entity): boolean {
		return (
			groundHeight(e.pos.x, e.pos.z, this.cfg.seed) <
				WATER_LEVEL - SWIM_DEPTH && e.pos.y <= SWIM_SURFACE_Y + 0.15
		);
	}
	...
	private updateChargeMovement(p: Entity): boolean {
		...
		if (!target || target.dead || p.chargeTimeLeft <= 0 || this.isRooted(p))
			return done(false);
	...
	private updateFollowMovement(p: Entity, meta: PlayerMeta): boolean {
		...
		if (this.isStunned(p) || this.isRooted(p) || d <= FOLLOW_STOP_DIST)
			return true;
		...
		if (h1 > h0 && step > 1e-5 && (h1 - h0) / step > MAX_CLIMB_SLOPE)
	...
	private updateRegen(p: Entity, meta: PlayerMeta): void {
		...
		for (const slot of ["eating", "drinking"] as const) {
			...
			if (c.hpPer2s > 0 && p.hp < p.maxHp) {
				const heal = Math.min(
					Math.round(c.hpPer2s * this.healingTakenMult(p)),
					p.maxHp - p.hp,
				);
				p.hp += heal;
	...
	private updateGroundAoEs(): void {
		for (let i = this.groundAoEs.length - 1; i >= 0; i--) {
			...
			effect.tickTimer -= DT;
			while (
				effect.tickTimer <= CAST_COMPLETE_EPS &&
	...
	private updateCasting(p: Entity, meta: PlayerMeta): void {
		...
		if (p.castingAbility !== FISHING_CAST_ID) {
			...
			if (
				cast &&
				cast.def.school !== "physical" &&
				this.isLockedOut(p, cast.def.school)
			) {
				this.cancelCast(p);
				return;
			}
	...
	private lineOfSightBlocked(
		...
		return (
			this.abilityNeedsLineOfSight(ability) &&
	...
	private pushbackCast(p: Entity): void {
		if (p.channeling) {
			p.castRemaining = Math.max(
				0,
				p.castRemaining - p.castTotal * CHANNEL_PUSHBACK_FRACTION,
			);
		} else {
	...
	castAbility(abilityId: string, pid?: number): void {
		...
		const togglingOff =
			isToggleBuff(ability) && p.auras.some((a) => a.id === ability.id);
		...
		if (
			ability.spendsCombo &&
			(p.comboPoints <= 0 || p.comboTargetId !== p.targetId)
		) {
		...
		if (ability.requiresTarget && ability.targetType === "friendly") {
			...
			const cur =
				p.targetId !== null ? (this.entities.get(p.targetId) ?? null) : null;
			...
			if (!target || target.dead || !this.isHostileTo(p, target)) {
				this.error(
					p.id,
					"You have no target.",
					target?.dead ? "target_dead" : undefined,
				);
				return;
			}
			const d = dist2d(p.pos, target.pos);
			...
			const facingDiff = Math.abs(
				normAngle(angleTo(p.pos, target.pos) - p.facing),
			);
			...
			for (const eff of res.effects) {
				if (eff.type === "weaponStrike" && eff.requiresBehind) {
					...
					const behindDiff = Math.abs(
						normAngle(angleTo(target.pos, p.pos) - target.facing),
					);
		...
		if (
			...
			p.auras.some((a) => a.id === "ghost_wolf")
		) {
	...
	private spendAbilityCost(p: Entity, res: ResolvedAbility): void {
		if (isToggleBuff(res.def) && p.auras.some((a) => a.id === res.def.id))
			return;
	...
	private consumeHealAbsorb(target: Entity, healed: number): number {
		...
		if (depleted)
			target.auras = target.auras.filter(
				(a) => !(a.kind === "heal_absorb" && a.value <= 0),
			);
		return remaining;
	}
	...
	private applyAbility(
		...
		if (ability.id === "conjure_water") {
			...
			this.addItem(
				res.rank > 1 && ITEMS[tiered] ? tiered : "conjured_water",
				2,
				p.id,
			);
			return;
		...
		if (ability.id === "conjure_food") {
			...
			this.addItem(
				res.rank > 1 && ITEMS[tiered] ? tiered : "conjured_bread",
				2,
				p.id,
			);
			return;
			...
			switch (eff.type) {
				...
				case "heal": {
					...
					this.applyHeal(
						p,
						healTarget,
						this.rng.range(eff.min, eff.max),
						ability.name,
					);
					break;
				}
				case "hot": {
					...
					this.applyAura(hotTarget, {
						...
						value: Math.max(
							1,
							Math.round(eff.total / (eff.duration / eff.interval)),
						),
				...
				case "judgement": {
					if (!target) break;
					const sealIdx = p.auras.findIndex(
						(a) => a.kind === "imbue" && a.value2 !== undefined,
					);
					if (sealIdx < 0) {
						this.error(p.id, "You have no active Seal.");
						break;
				...
				case "incapacitate": {
					...
					const remaining =
						ability.id === "fear"
							? this.diminishedCrowdControlDuration(
									p,
									target,
									"fear",
									eff.duration,
					...
					this.applyAura(target, {
						...
						duration: remaining,
						value:
							ability.id === "fear" ? this.rng.range(-Math.PI, Math.PI) : 0,
						sourceId: p.id,
						school: ability.school,
						breaksOnDamage: true,
					});
				...
				case "charge": {
					...
					if (p.resourceType === "rage")
						p.resource = Math.min(p.maxResource, p.resource + 9);
					this.enterCombat(p, target);
					break;
				}
				case "sunder": {
					...
					addThreat(
						target,
						p.id,
						res.threatFlat * this.threatMod(p, "physical"),
					);
					this.enterCombat(p, target);
					break;
				}
				...
				case "dismissPet": {
					...
					this.error(
						p.id,
						"Permanent pets can only be abandoned from the pet frame.",
					);
					break;
				}
				case "summonDemon": {
	...
	private diminishedCrowdControlDuration(
		...
		if (
			source.kind !== "player" ||
			target.kind !== "player" ||
			!this.isHostileTo(source, target)
		) {
			return duration;
		}
		...
		if (category === "polymorph") {
			target.ccDr.set(category, {
				stage: stage + 1,
				resetAt: this.time + reset,
			});
			return PVP_POLYMORPH_DR_DURATIONS[
				Math.min(stage, PVP_POLYMORPH_DR_DURATIONS.length - 1)
			];
		}
		if (category === "fear") {
			target.ccDr.set(category, {
			...
			return PVP_FEAR_DR_DURATIONS[
				Math.min(stage, PVP_FEAR_DR_DURATIONS.length - 1)
			];
		}
	...
	private restorePet(owner: Entity, state: PetState): void {
		...
		if (state.dead) {
			...
			pet.hp = Math.max(
				1,
				Math.min(pet.maxHp, Math.round(state.hp) || pet.maxHp),
			);
		}
		this.addEntity(pet);
	}
	...
	private syncPetLevel(owner: Entity): void {
		...
		pet.hp = pet.dead
			? 0
			: Math.max(1, Math.min(pet.maxHp, Math.round(pet.maxHp * hpFrac)));
	}
	...
	private tameError(p: Entity, target: Entity): string | null {
		if (target.kind !== "mob" || !target.hostile)
			return "You cannot tame that.";
		const template = MOBS[target.templateId];
		if (!template || (template.type !== "beast" && template.type !== "spider"))
		...
		if (target.level > p.level)
			return "That beast is too high level for you to tame.";
		if (target.spawnPos.x > DUNGEON_X_THRESHOLD)
			return "You cannot tame dungeon creatures.";
	...
	private completeTame(p: Entity, target: Entity): void {
		...
		for (const m of this.entities.values()) {
			...
			if (m.aggroTargetId === target.id && !m.dead && m.aiState !== "dead")
				this.retargetMob(m);
		}
		this.addEntity(pet);
		this.syncPetLevel(p);
	...
	private despawnPersistentPet(pet: Entity): void {
		...
		for (const m of this.entities.values()) {
			...
			if (m.aggroTargetId === pet.id && !m.dead && m.aiState !== "dead")
				this.retargetMob(m);
		}
	...
	renamePet(name: string, pid?: number): void {
		...
		this.emit({
			type: "log",
			text: `Your pet is now named ${clean}.`,
			color: "#8f8",
			pid: r.e.id,
		});
	}
	...
	petAttack(pid?: number): void {
		...
		const target =
			r.e.targetId !== null ? this.entities.get(r.e.targetId) : null;
	...
	healPet(pid?: number): void {
		...
		r.e.gcdRemaining = Math.max(
			r.e.gcdRemaining,
			this.playerGcdFor(r.meta.cls),
		);
		this.emit({
			type: "log",
	...
	private createDemonPet(
		...
		const pos = this.groundPos(
			owner.pos.x + Math.sin(ang) * 2,
			owner.pos.z + Math.cos(ang) * 2,
		);
		...
		if (p.queuedOnSwing) {
			...
			if (queued) {
				...
				if (p.resource >= queued.cost && eff && eff.type === "weaponDamage") {
					...
					if (queued.def.cooldown > 0)
						p.cooldowns.set(queued.def.id, queued.def.cooldown);
					bonus = eff.bonus;
	...
	private rangedSwing(
		...
		const missChance =
			meleeMissChance(attacker.level, target.level) +
			this.blindMissBonus(attacker);
		...
		let dmg =
			this.rng.range(ranged.min, ranged.max) +
			(attacker.rangedPower / 14) * ranged.speed;
	...
	private meleeSwing(
		...
		let imbueBonus = 0;
		for (const a of attacker.auras)
			if (a.kind === "imbue") imbueBonus += a.value;
		let dmg =
	...
	private dealDamage(
		...
		if (school === "physical" && amount > 0) {
			let exposeMult = 1;
			for (const a of target.auras)
				if (a.kind === "expose") exposeMult += a.value;
		...
		if (amount > 0) {
			let vuln = 0;
			for (const a of target.auras)
				if (a.kind === "vulnerability") vuln += a.value;
		...
		const duel =
			target.kind === "player" ? this.duels.get(target.id) : undefined;
		if (
			duel &&
			duel.state === "active" &&
			sourcePlayer &&
		...
		const match =
			target.kind === "player" ? this.arenaMatches.get(target.id) : undefined;
		...
		if (
			...
			const ls =
				this.players.get(sourcePlayer.id)?.fiestaSpecial.lifestealPct ?? 0;
			if (
				ls > 0 &&
				!sourcePlayer.dead &&
		...
		if (amount > 0) {
			if (
				target.kind === "mob" &&
				DAMAGE_IDLE_DESPAWN_MOB_IDS.has(target.templateId)
			) {
		...
		if (target.kind === "player") {
			...
			if (
				...
				else if (!ignoresDamagePushback(target.castingAbility))
	...
	private maybeFrenzyOnHit(target: Entity, source: Entity | null): void {
		...
		if (target.kind !== "mob" || !target.hostile || target.ownerId !== null)
			return;
	...
	private handleDeath(e: Entity, killer: Entity | null): void {
		...
		if (e.kind === "mob") {
			...
			e.respawnTimer =
				this.cfg.respawnSeconds *
				(template?.respawnMult ?? (template?.rare ? 4 : 1));
			e.aggroTargetId = null;
			clearThreat(e);
			if (e.ownerId !== null) {
			...
			const creditId =
				e.tappedById ?? (killer?.kind === "player" ? killer.id : null);
			const meta = creditId !== null ? this.players.get(creditId) : null;
			const creditEntity =
				creditId !== null ? this.entities.get(creditId) : null;
			if (meta && creditEntity) {
				...
				if (party) {
					for (const mPid of party.members) {
						...
						if (
							...
							mE &&
							!mE.dead &&
							dist2d(mE.pos, e.pos) <= PARTY_XP_RANGE
						)
							eligible.push(mMeta);
					}
				}
				...
				const bonus =
					GROUP_XP_BONUS[Math.min(eligible.length, GROUP_XP_BONUS.length) - 1];
				...
				for (const member of eligible) {
					...
					const xpGain = Math.round(
						(mobXpValue(e.level, mE.level) * eliteMult * bonus) /
							eligible.length,
					);
	...
	private updateRested(p: Entity, meta: PlayerMeta): void {
		...
		const perSecond =
			(RESTED_FILL_FRACTION * xpForLevel(p.level)) / fillSeconds;
	...
	private accrueLifetimeXp(amount: number, meta: PlayerMeta, p: Entity): void {
		...
		if (meta.lifetimeXp >= Number.MAX_SAFE_INTEGER) {
			...
			console.warn(
				`lifetimeXp for ${meta.name} hit the 2^53 ceiling and was clamped`,
			);
		}
		if (atCap) {
	...
	private checkMilestones(meta: PlayerMeta, p: Entity): void {
		for (const m of MILESTONES) {
			if (
				meta.lifetimeXp >= m.lifetimeXp &&
				!meta.unlockedMilestones.has(m.id)
			) {
	...
	prestige(pid?: number): boolean {
		...
		if (!canPrestige(r.e.level, r.meta.lifetimeXp, r.meta.prestigeRank))
			return false;
		r.meta.xp = 0;
	...
	private needsQuestDrop(entry: LootEntry, meta: PlayerMeta): boolean {
		...
		return (
			objIdx >= 0 &&
			this.countItem(entry.itemId, meta.entityId) <
				quest.objectives[objIdx].count
		);
	}
	...
	private rollLoot(
		...
		for (const entry of template.loot) {
			...
			if (entry.rollGroup) {
				...
				const group = template.loot.filter(
					(l) => l.rollGroup === entry.rollGroup,
				);
				const roll = this.rng.next();
				let cumulative = 0;
			...
			if (entry.questId) {
				const questRecipients = eligible.filter((m) =>
					this.needsQuestDrop(entry, m),
				);
	...
	private tryAwardCopperByFairSplit(mob: Entity, copper: number): boolean {
		...
		const shares = new Map<PlayerMeta, number>(
			candidates.map((candidate) => [candidate, base]),
		);
	...
	private distributeLootCopper(mob: Entity, looter: PlayerMeta): void {
		...
		if (!this.tryAwardCopperByFairSplit(mob, copper))
			this.awardAllCopperToLooter(looter, copper);
		mob.loot.copper = 0;
	}

	private startNeedGreedRoll(itemId: string, mob: Entity): boolean {
		if (this.effectiveItemLootStrategy(itemId, mob) !== "need-greed")
			return false;
		const candidates = this.partyLootCandidatesForMob(mob);
		if (candidates.length <= 1) return false;
	...
	private awardSharedLootItem(
		itemId: string,
		mob: Entity,
		looter: PlayerMeta,
	): void {
		if (!this.startNeedGreedRoll(itemId, mob))
	...
	submitLootRoll(rollId: number, choice: LootRollChoice, pid?: number): void {
		...
		roll.choices.set(r.meta.entityId, {
			choice,
			roll:
				choice === "need" || choice === "greed" ? this.rng.int(1, 100) : null,
		});
	...
	private resolveLootRoll(roll: PendingLootRoll): void {
		...
		const contenders =
			needers.length > 0
				? needers
				: entries.filter((entry) => entry.result.choice === "greed");
		if (contenders.length === 0) {
			this.returnLootRollItemToCorpse(roll);
			for (const pid of roll.candidates)
				this.emit({
					type: "loot",
					text: `Everyone passed on ${roll.itemName}.`,
					pid,
				});
			return;
		}
		...
		for (const contender of contenders.slice(1)) {
			if ((contender.result.roll ?? 0) > (winner.result.roll ?? 0))
				winner = contender;
		}
	...
	private returnLootRollItemToCorpse(roll: PendingLootRoll): void {
		...
		const existing = mob.loot.items.find(
			(slot) =>
				slot.openToAll && slot.itemId === roll.itemId && !slot.personalFor,
		);
		if (existing) existing.count += 1;
		else
			mob.loot.items.push({ itemId: roll.itemId, count: 1, openToAll: true });
		mob.lootable = true;
	}

	private lootSlotVisibleTo(slot: LootSlot, pid: number): boolean {
		return (
			slot.openToAll || !slot.personalFor || slot.personalFor.includes(pid)
		);
	}

	private hasPendingLootRollForMob(mobId: number): boolean {
		return [...this.pendingLootRolls.values()].some(
			(roll) => roll.mobId === mobId,
		);
	}
	...
	private findNythraxisBossForAdd(add: Entity): Entity | null {
		...
		for (const e of this.entities.values()) {
			if (e.kind !== "mob" || e.templateId !== NYTHRAXIS_BOSS_ID || e.dead)
				continue;
			if (
				e.summonedIds.includes(add.id) ||
				dist2d(e.spawnPos, add.spawnPos) < 1
			)
				return e;
		}
		return null;
	...
	private nythraxisAddFallbackTarget(add: Entity): Entity | null {
		...
		const target =
			boss.aggroTargetId !== null
				? this.entities.get(boss.aggroTargetId)
				: null;
	...
	private scheduleNythraxisAddDespawnIfBossReset(add: Entity): boolean {
		...
		if (
			!boss ||
			(boss.inCombat && boss.aiState !== "idle" && boss.aiState !== "evade")
		)
			return false;
	...
	private tryMobMeleeSwingInRange(mob: Entity, target: Entity): boolean {
		if (dist2d(mob.pos, target.pos) > this.mobEffectiveMeleeRange(mob, target))
			return false;
		mob.aiState = "attack";
	...
	private mobShouldResetCombat(mob: Entity, target: Entity): boolean {
		...
		if (mob.fleeReturnTimer > 0) {
			...
			if (dist2d(mob.pos, leashAnchor) <= WORLD_LEASH_DISTANCE - 1)
				mob.fleeReturnTimer = 0;
		}
		if (
			policy.shouldReset(runtime, target, leashAnchor) &&
			mob.fleeReturnTimer <= 0
		) {
			this.resetMobCombat(mob);
			return true;
	...
	private updateMobMeleePursuit(mob: Entity, target: Entity): void {
		...
		this.tryMobMeleeSwingInRange(mob, target);
		mob.aiState =
			dist2d(mob.pos, target.pos) <= profile.meleeRange ? "attack" : "chase";
	}
	...
	private updateMob(mob: Entity): void {
		if (mob.dead) {
			if (
				mob.templateId === NYTHRAXIS_BOSS_ID &&
				mob.nythraxis &&
				!mob.nythraxis.deathSpoken
			) {
		...
		switch (mob.aiState) {
			...
			case "chase": {
				...
				const target =
					mob.aggroTargetId !== null
						? this.entities.get(mob.aggroTargetId)
						: null;
			...
			case "attack": {
				...
				if (stomp) {
					...
					if (mob.stompTimer <= 0) {
						...
						for (const meta of this.players.values()) {
							const pe = this.entities.get(meta.entityId);
							if (!pe || pe.dead || dist2d(pe.pos, mob.pos) > stomp.radius)
								continue;
				...
				if (terrify) {
					...
					if (mob.terrifyTimer <= 0) {
						...
						for (const meta of this.players.values()) {
							const pe = this.entities.get(meta.entityId);
							if (!pe || pe.dead || dist2d(pe.pos, mob.pos) > terrify.radius)
								continue;
							const remaining = this.diminishedCrowdControlDuration(
								mob,
								pe,
	...
	private mobSwing(mob: Entity, target: Entity): void {
		...
		if (
			...
			const heal = Math.min(
				mob.maxHp - mob.hp,
				Math.max(1, Math.round(dealt * leech.healFrac)),
			);
			if (heal > 0) {
				mob.hp += heal;
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(soulrot.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(frostbite.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(smolder.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(arcaneRot.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(stackPoison.chance)
		) {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(corrode.chance)
		) {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(silence.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(lockout.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(costTax.chance)
		) {
			this.applyAura(target, {
		...
		if (
			...
			mob.hostile &&
			!target.dead &&
			this.rng.chance(slowStrike.chance)
		) {
			this.applyAura(target, {
	...
	private devourBeneficialAura(target: Entity, name: string): boolean {
		...
		const meta = this.players.get(target.id);
		if (meta)
			recalcPlayerStats(target, meta.cls, meta.equipment, meta.talentMods);
		this.emit({ type: "aura", targetId: target.id, name, gained: true });
		return true;
	}
	...
	private updatePet(pet: Entity): void {
		...
		if (!pet.inCombat && this.tickCount % 40 === 0 && pet.hp < pet.maxHp) {
			pet.hp = Math.min(
				pet.maxHp,
				pet.hp + Math.max(1, Math.round(pet.maxHp * 0.02)),
			);
		}
	...
	private petFollow(pet: Entity, owner: Entity): void {
		...
		while (
			...
			dist2d(pet.pos, pet.petPath[0]) < PET_WAYPOINT_REACHED
		)
			pet.petPath.shift();
		...
		const speed =
			Math.max(pet.moveSpeed, RUN_SPEED * 1.1) * this.moveSpeedMult(pet);
	...
	private petPickTarget(pet: Entity, owner: Entity): Entity | null {
		...
		const ownerIdle =
			!ownerMeta ||
			this.tickCount - ownerMeta.lastActiveTick > PET_OWNER_IDLE_TICKS;
		...
		for (const m of this.entities.values()) {
			...
			const engagingUs =
				m.kind === "mob" &&
				(m.aggroTargetId === owner.id || m.aggroTargetId === pet.id);
			const ownerOffense =
				owner.targetId === m.id &&
		...
		if (tmpl.mendAlly) {
			...
			if (mob.mendTimer <= 0) {
				...
				for (const ally of this.entities.values()) {
					if (ally.kind !== "mob" || ally.dead || ally.ownerId !== null)
						continue; // skip players, pets, corpses
	...
	private initNythraxisEncounter(
		boss: Entity,
	): NonNullable<Entity["nythraxis"]> {
		if (!boss.nythraxis) {
			boss.nythraxis = {
				phase: 1,
				introSpoken: false,
	...
	private updateNythraxisEncounter(boss: Entity): void {
		...
		if (!st.introSpoken) {
			...
			this.nythraxisDialogueSet(boss, [
				{
					speaker: "nythraxis",
					text: "Another kingdom comes to challenge me",
					delay: 0,
				},
				{
					speaker: "nythraxis",
		...
		if (
			...
			boss.inCombat = true;
			if (boss.aiState === "idle" || boss.aiState === "evade")
				boss.aiState = "chase";
		}
		if (
			boss.aggroTargetId !== null &&
			(boss.aiState === "idle" || boss.aiState === "evade")
		) {
			boss.inCombat = true;
			boss.aiState = "chase";
		}

		if (st.soulRendLockout > 0)
			st.soulRendLockout = Math.max(0, st.soulRendLockout - DT);
		...
		if (
			st.phase === 2 &&
			!st.finalStand &&
			hpFrac <= NYTHRAXIS_FINAL_STAND_HP
		) {
			st.finalStand = true;
			boss.enraged = true;
			this.nythraxisDialogueSet(boss, [
		...
		if (st.phase === 2) {
			...
			if (st.soulRendTimer <= 0) {
				if (this.canCastNythraxisSoulRend(st))
					this.castNythraxisSoulRend(boss, st);
	...
	private reserveNythraxisDialogue(
		...
		const delay =
			!critical && queue && busyUntil > this.time ? busyUntil - this.time : 0;
	...
	private nythraxisDialogueSet(
		...
		const duration =
			Math.max(...lines.map((line) => line.delay)) +
	...
	private nythraxisYellEvent(
		...
		const from =
			actor?.name ?? (speaker === "aldric" ? "Brother Aldric" : boss.name);
		...
		return {
			...
			text,
			channel: "yell",
			entityId: actor?.id ?? boss.id,
		};
	}

	private emitNythraxisYell(
		boss: Entity,
		speaker: "nythraxis" | "aldric",
		text: string,
	): void {
	...
	private playersInNythraxisRoom(boss: Entity): Entity[] {
		...
		for (const meta of this.players.values()) {
			const p = this.entities.get(meta.entityId);
			if (p && !p.dead && dist2d(p.pos, boss.spawnPos) <= NYTHRAXIS_ROOM_RADIUS)
				out.push(p);
		}
	...
	private nythraxisTransitionStunTargets(boss: Entity): Entity[] {
		return [...this.entities.values()].filter(
			(e) =>
				...
				(e.kind === "player" ||
					(e.kind === "mob" &&
						(e.templateId === NYTHRAXIS_ADD_ID || e.ownerId !== null))),
		);
	}
	...
	private nythraxisRoomMetas(boss: Entity): PlayerMeta[] {
		...
		for (const meta of this.players.values()) {
			const p = this.entities.get(meta.entityId);
			if (p && dist2d(p.pos, boss.spawnPos) <= NYTHRAXIS_ROOM_RADIUS)
				out.push(meta);
		}
	...
	private updateNythraxisGravebreaker(
		boss: Entity,
		...
		for (const p of this.playersInNythraxisRoom(boss)) {
			...
			const mitigated =
				rawDmg *
				mult *
				(1 - armorReduction(this.effectiveArmor(p), boss.level));
	...
	private updateNythraxisRaiseFallen(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
	...
	private spawnNythraxisAdds(boss: Entity): void {
		...
		const inst = this.instances.find(
			(i) => i.partyKey !== null && i.mobIds.includes(boss.id),
		);
		const victimId =
			boss.aggroTargetId ?? threatEntries(boss, 1)[0]?.[0] ?? null;
		...
		for (const pos of spawnPoints) {
			...
			if (victim && !victim.dead && victim.kind === "player")
				this.aggroMob(add, victim, false);
		}
		this.emit({
			type: "spellfx",
			sourceId: boss.id,
	...
	private startNythraxisTransition(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
		st.phase = "transition";
		st.transitionStarted = true;
		const queuedDialogueDelay = Math.max(
			0,
			(st.dialogueBusyUntil ?? 0) - this.time,
		);
		...
		const transitionLines = [
			...
			{
				speaker: "aldric" as const,
				text: "Your kingdom is gone, Nythraxis",
				delay: 3.0,
			},
			{
				speaker: "aldric" as const,
				text: "Yet you still cling to it",
				delay: 5.7,
			},
			{
				speaker: "aldric" as const,
				text: "Champions, listen carefully!",
				delay: 8.4,
			},
			{
				speaker: "aldric" as const,
				text: "The wardstones still bind his soul.",
				delay: 11.2,
			},
			{
				speaker: "aldric" as const,
				text: "When the time comes, do not ignore them.",
				delay: 14.1,
			},
			{
				speaker: "aldric" as const,
				text: "Fail and we all perish",
				delay: 17.1,
			},
		];
		this.emit({
	...
	private spawnNythraxisAldric(boss: Entity): void {
		...
		const aldric = createNpc(
			...
			this.groundPos(
				boss.spawnPos.x,
				boss.spawnPos.z - NYTHRAXIS_ALDRIC_SPAWN_DIST,
			),
		);
	...
	private updateNythraxisTransition(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
		const aldric = this.findNythraxisAldric(boss);
		if (aldric) {
			const dest = this.groundPos(
				boss.spawnPos.x,
				boss.spawnPos.z - NYTHRAXIS_ALDRIC_WALK_DIST,
			);
		...
		boss.auras = boss.auras.filter(
			(a) => a.id !== "nythraxis_transition_pause",
		);
		for (const e of this.nythraxisTransitionStunTargets(boss)) {
			e.auras = e.auras.filter((a) => a.id !== "nythraxis_transition_stun");
		}
	...
	private canCastNythraxisSoulRend(
		st: NonNullable<Entity["nythraxis"]>,
	): boolean {
		return st.deathlessCastRemaining <= 0 && st.deathlessStunRemaining <= 0;
	}

	private castNythraxisSoulRend(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
		const candidates = this.playersInNythraxisRoom(boss).filter(
			(p) => p.id !== boss.aggroTargetId,
		);
	...
	private updateNythraxisSoulRend(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
	...
	private startNythraxisDeathlessRage(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
		...
		st.wardChannels = this.nythraxisDeathlessChannelObjects(boss).map(
			(ward) => ({
				objectId: ward.id,
				playerId: null,
	...
	private updateNythraxisDeathlessRage(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
	...
	private nythraxisWardstoneInterruptReady(
		st: NonNullable<Entity["nythraxis"]>,
	): boolean {
		if (
			...
			return false;
		return (
			new Set(st.wardChannels.map((c) => c.playerId)).size ===
			st.wardChannels.length
		);
	}

	private updateNythraxisWardChannels(
		boss: Entity,
		st: NonNullable<Entity["nythraxis"]>,
	): void {
	...
	private tryStartNythraxisWardChannel(ward: Entity, player: Entity): boolean {
		...
		if (!boss.nythraxis || boss.nythraxis.deathlessCastRemaining <= 0)
			return true;
		const channel = boss.nythraxis.wardChannels.find(
			(c) => c.objectId === ward.id,
		);
		if (!channel || channel.complete) return true;
		if (channel.playerId === player.id) return true;
		if (channel.playerId !== null && channel.playerId !== player.id)
			return true;
	...
	private isEnemyTargetCandidate(attacker: Entity, target: Entity): boolean {
		...
		if (target.kind === "mob" && target.ownerId !== null) {
			...
			return (
				!!owner &&
				owner.kind === "player" &&
				this.isEnemyTargetCandidate(attacker, owner)
			);
		}
	...
	equipItem(itemId: string, pid?: number): void {
		...
		const { meta, e: p } = r;
		const def = ITEMS[itemId];
		if (!def || !def.slot || (def.kind !== "weapon" && def.kind !== "armor"))
			return;
	...
	private isAtDeepfenShallowsFishingSpot(p: Entity): boolean {
		const d = Math.hypot(
			p.pos.x - DEEPFEN_SHALLOWS_LAKE.x,
			p.pos.z - DEEPFEN_SHALLOWS_LAKE.z,
		);
	...
	private completeFishing(p: Entity, meta: PlayerMeta): void {
		...
		const table =
			FISHING_TABLES[zoneAt(p.pos.z).id] ?? FISHING_TABLES.eastbrook_vale;
	...
	useItem(itemId: string, pid?: number): ItemUseResult | void {
		...
		if (def.kind === "food" || def.kind === "drink") {
			...
			p[slot] = {
				...
				manaPer2s: def.drinkMana
					? Math.round(def.drinkMana / CONSUME_TICKS)
					: 0,
			...
			if (restoresHp) {
				const heal = Math.min(
					Math.round(def.potionHp! * this.healingTakenMult(p)),
					p.maxHp - p.hp,
				);
				p.hp += heal;
				this.emit({ type: "heal", targetId: p.id, amount: heal });
	...
	sellItem(itemId: string, count = 1, pid?: number): void {
		...
		const sellCount = Number.isFinite(count)
			? Math.min(Math.floor(count), available)
			: 0;
	...
	buyBackItem(itemId: string, pid?: number): void {
		...
		slot.count -= 1;
		if (slot.count <= 0)
			meta.vendorBuyback = meta.vendorBuyback.filter((s) => s !== slot);
		this.addItemSilent(itemId, 1, meta);
		this.onInventoryChangedForQuests(meta);
		this.emit({
			type: "vendor",
	...
	private maybeAutoEquip(itemId: string, meta: PlayerMeta): void {
		...
		if (def.kind === "weapon") {
			...
			const cur = meta.equipment[def.slot]
				? ITEMS[meta.equipment[def.slot]!]
				: null;
	...
	lootCorpse(mobId: number, pid?: number): void {
		...
		const tapperParty =
			mob.tappedById !== null ? this.partyOf(mob.tappedById) : null;
		const hasSharedLootRights =
			mob.tappedById === null ||
			mob.tappedById === meta.entityId ||
			!!tapperParty?.members.includes(meta.entityId);
		const hasPersonalLoot = mob.loot.items.some((s) =>
			s.personalFor?.includes(meta.entityId),
		);
	...
	pickUpObject(objId: number, pid?: number): void {
		...
		if (!obj || obj.kind !== "object" || !obj.lootable || !obj.objectItemId)
			return;
		...
		if (def?.questId) {
			...
			if (!qp || (qp.state !== "active" && qp.state !== "ready")) {
				this.error(
					meta.entityId,
					def.pickupDeny ?? `You cannot take the ${def.name} yet.`,
				);
				return;
			}
			...
			if (objIdx < 0) {
				this.error(
					meta.entityId,
					def.pickupEnough ?? `${def.name} offers nothing more.`,
				);
				return;
			}
			if (
				objIdx >= 0 &&
				this.countItem(obj.objectItemId, meta.entityId) >=
					quest.objectives[objIdx].count
			) {
				this.error(
					meta.entityId,
					def.pickupEnough ?? "You have enough of those.",
				);
				return;
			}
		}
	...
	private activateNythraxisRelic(obj: Entity, meta: PlayerMeta): boolean {
		...
		if (!qp || qp.state !== "active") {
			...
			this.error(
				meta.entityId,
				def?.pickupDeny ?? "The relic is bound by the sealed crypt.",
			);
			return true;
		}
		...
		if (
			objectiveIndex >= 0 &&
			this.countItem(obj.objectItemId, meta.entityId) >=
				quest.objectives[objectiveIndex].count
		) {
			const def = ITEMS[obj.objectItemId];
			this.error(
				meta.entityId,
				def?.pickupEnough ?? "You have already recovered this relic.",
			);
			return true;
		}
		...
		for (const qp of meta.questLog.values()) {
			...
			quest.objectives.forEach((objective, objectiveIndex) => {
				if (
					...
					objective.targetObjectItemId !== obj.objectItemId
				)
					return;
				handled = true;
				...
				if (
					...
					!this.countItem("crypt_keystone", meta.entityId)
				) {
					this.error(
						meta.entityId,
						"The ritual circle is silent without the Crypt Keystone.",
					);
					return;
				}
				...
				if (obj.objectItemId === "crypt_ritual_circle")
	...
	private sharedNythraxisObjectParticipants(
		...
		for (const pid of members) {
			...
			if (
				...
				entity.dead ||
				!memberQp ||
				memberQp.state !== "active"
			)
				continue;
			if (memberQp.counts[objectiveIndex] >= objective.count) continue;
			if (dist2d(entity.pos, obj.pos) > NYTHRAXIS_PARTY_INTERACT_RANGE)
				continue;
			eligible.push(member);
		}
		return eligible.some((member) => member.entityId === actor.entityId)
			? eligible
			: [actor];
	}

	private emitQuestObjectVision(
		itemId: string,
		pids: number[],
		...
		for (let i = 0; i < lines.length; i++) {
			for (const pid of pids) {
				...
				if (i === 0) this.emit(event);
					...
					this.delayedEvents.push({
						at: this.time + i * NYTHRAXIS_VISION_LINE_DELAY,
						event,
					});
			}
		}
	...
	private summonQuestMob(
		...
		if (owner && owner.kind === "player" && !owner.dead)
			this.aggroMob(mob, owner, false);
		const inst = this.instances.find((i) => {
			if (i.partyKey === null) return false;
			const origin = this.instanceOriginOf(i);
			return (
				Math.abs(mob.pos.x - origin.x) < 120 &&
				Math.abs(mob.pos.z - origin.z) < 250
			);
		});
	...
	private questNpcFor(
		...
		const quest = QUESTS[questId];
		const templateIds =
			role === "giver" ? [quest.giverNpcId] : questTurnInNpcIds(quest);
		let sawNpc = false;
		for (const e of this.entities.values()) {
			if (
				!this.isQuestInteractionEntity(e) ||
				!templateIds.includes(e.templateId)
			)
				continue;
			if (role === "giver" && e.kind !== "npc") continue;
			sawNpc = true;
			if (dist2d(p.pos, e.pos) <= INTERACT_RANGE + 2)
				return { npc: e, tooFar: false };
		}
	...
	private onMobKilledForQuests(mob: Entity, meta: PlayerMeta): void {
		for (const qp of meta.questLog.values()) {
			...
			quest.objectives.forEach((obj, i) => {
				if (
					obj.type === "kill" &&
					obj.targetMobId === mob.templateId &&
		...
		for (const qp of meta.questLog.values()) {
			...
			quest.objectives.forEach((obj, i) => {
				if (obj.type === "collect" && obj.itemId) {
					const have = Math.min(
						obj.count,
						this.countItem(obj.itemId, meta.entityId),
					);
					if (have !== qp.counts[i]) {
						if (have > qp.counts[i])
							meta.counters.questProgress += have - qp.counts[i];
						qp.counts[i] = have;
						changed = true;
						this.emit({
							type: "questProgress",
	...
	private chatAllowed(pid: number): boolean {
		...
		b.tokens = Math.min(
			CHAT_BURST,
			b.tokens + (this.time - b.at) * CHAT_REFILL,
		);
		b.at = this.time;
	...
	private handleDevChat(raw: string, pid: number): SentChat | null | undefined {
		...
		const tpM =
			/^\/(?:dev\s+tp|devtp)\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s*$/i.exec(
				raw,
			);
		if (tpM) {
			const e = this.entities.get(pid);
		...
		const giveM = /^\/(?:dev\s+give|devgive)\s+(\S+)(?:\s+(\d+))?\s*$/i.exec(
			raw,
		);
		if (giveM) {
	...
	private resolveWhisperTarget(
		...
		for (const target of this.players.values()) {
			const exactMessage = this.whisperMessageForName(
				trimmed,
				target.name,
				true,
			);
			...
			const insensitiveMessage = this.whisperMessageForName(
				trimmed,
				target.name,
				false,
			);
		...
		const longest = matches.filter(
			(m) => m.target.name.length === longestLength,
		);
		...
		if (longest.length > 1)
			return {
				error: `Several players match '${typedName}'. Use exact capitalization.`,
			};
	...
	chat(text: string, pid?: number): SentChat | null {
		...
		if (awaym) {
			...
			if (r.meta.away?.mode === mode && !custom) {
				...
				const message =
					custom || (mode === "afk" ? "Away From Keyboard" : "Do Not Disturb");
		...
		if (im) {
			...
			if (!target || !te) {
				this.error(
					r.meta.entityId,
					`There is no player named '${targetName}' online.`,
				);
				return null;
			}
		...
		if (/^\/unfollow(?:\s|$)/i.test(raw)) {
			if (r.e.followTargetId === null)
				this.error(r.meta.entityId, "You are not following anyone.");
		...
		if (fm) {
			...
			if (nameArg) {
				...
				if (!target) {
					this.error(
						r.meta.entityId,
						`There is no player named '${nameArg}' online.`,
					);
					return null;
				}
			} else {
				const cur =
					r.e.targetId !== null ? this.players.get(r.e.targetId) : undefined;
				if (!cur) {
					this.error(
						r.meta.entityId,
		...
		if (/^\/played(?:\s|$)/i.test(raw)) {
			...
			this.error(
				r.meta.entityId,
				`Time played this session: ${parts.join(" ")}.`,
			);
			return null;
		}
		...
		if (wm) {
			...
			if (target.away) {
				const label =
					target.away.mode === "afk" ? "Away From Keyboard" : "Do Not Disturb";
				this.emit({
					type: "log",
		...
		if (emMatch) {
			const key =
				EMOTE_ALIASES[emMatch[1].toLowerCase()] ?? emMatch[1].toLowerCase();
			...
			if (def) {
				...
				if (targetName && def.target) {
					...
					if (t)
						text = def.target.replace(
							"%t",
							t.name === r.meta.name ? "themselves" : t.name,
						);
				}
		...
		if (/^\/y(ell)?\s/i.test(raw)) {
			...
			this.error(
				r.meta.entityId,
				`Unknown command: ${raw.split(" ")[0]}. Type /help for a list.`,
			);
			return null;
		}
		if (!clean) return null;
	...
	partyInvite(targetPid: number, pid?: number): void {
		...
		if (myParty && myParty.members.length >= this.partyCapacity(myParty)) {
			this.error(
				r.meta.entityId,
				myParty.raid ? "Your raid is full." : "Your party is full.",
			);
			return;
		}
		...
		this.partyInvites.set(targetPid, {
			fromPid: r.meta.entityId,
			expires: this.time + 30,
		});
		this.emit({
			type: "partyInvite",
			fromPid: r.meta.entityId,
	...
	partyAccept(pid?: number): void {
		...
		if (party.members.length >= this.partyCapacity(party)) {
			this.error(
				r.meta.entityId,
				party.raid ? "That raid is full." : "That party is full.",
			);
			return;
		}
	...
	partyKick(targetPid: number, pid?: number): void {
		...
		if (!party.members.includes(targetPid) || targetPid === r.meta.entityId)
			return;
	...
	private nextRaidGroupFor(party: Party): 1 | 2 {
		const g1 = party.members.filter(
			(mPid) => (party.raidGroups.get(mPid) ?? 1) === 1,
		).length;
	...
	duelRequest(targetPid: number, pid?: number): void {
		...
		this.duelInvites.set(targetPid, {
			fromPid: r.meta.entityId,
			expires: this.time + 30,
		});
		this.emit({
	...
	private endDuel(duel: DuelState, winnerPid: number | null): void {
		...
		for (const e of [ea, eb]) {
			...
			if (
				e &&
				e.targetId !== null &&
				(e.targetId === duel.a || e.targetId === duel.b)
			) {
				e.autoAttack = false;
			}
		}
	...
	arenaQueueJoin(
		...
		if (!party || party.members.length === 1) {
			...
			if (party.leader !== id) {
				this.error(
					id,
					`Only the party leader may queue your team for ${label}.`,
				);
				return;
			}
		...
		const unit: ArenaQueueUnit = {
			pids: unitPids,
			rating: this.arenaTeamRating(unitPids, "2v2"),
		};
		queue.push(unit);
	...
	private arenaQueuedFormat(pid: number): ArenaFormat | null {
		...
		if (this.arenaQueueFiesta.some((u) => u.pids.includes(pid)))
			return "fiesta";
		return null;
	}
	...
	private arenaQueuePosition(pid: number, format: ArenaFormat): number {
		...
		const queue =
			format === "fiesta" ? this.arenaQueueFiesta : this.arenaQueue2v2;
		let pos = 0;
	...
	private arenaCombatants(pids: number[]): ArenaCombatant[] {
		...
		for (const pid of pids) {
			...
			const e = this.entities.get(pid);
			if (meta && e)
				out.push({ pid, name: meta.name, cls: meta.cls, level: e.level });
		}
		return out;
	}
			...
			if (match.state === "countdown") {
				...
				if (match.timer <= 0) {
					...
					for (const e of fighters)
						this.readyArenaFighter(e, { clearPrep: false });
					for (const mPid of this.arenaAllPids(match)) {
						this.emit({
							type: "log",
	...
	private matchmakeArena1v1(): void {
		...
		while (guard-- > 0) {
			...
			if (this.arenaQueue1v1.length < 2 || this.freeArenaSlot() === null)
				return;
	...
	private pruneTeamQueue(fmt: "2v2" | "fiesta"): void {
		...
		if (fmt === "fiesta")
			this.arenaQueueFiesta = this.arenaQueueFiesta.filter(keep);
		else this.arenaQueue2v2 = this.arenaQueue2v2.filter(keep);
	}

	private removeTeamQueueUnits(
	...
	private matchmakeTeamFormat(fmt: "2v2" | "fiesta"): void {
		...
		while (guard-- > 0) {
			...
			const queue =
				fmt === "fiesta" ? this.arenaQueueFiesta : this.arenaQueue2v2;
			...
			if (premades.length >= 1) {
				...
				if (solos.length >= 2) {
					...
					this.startArenaMatch(fmt, premade.pids, [
						anchorSolo.pids[0],
						partner.pids[0],
					]);
					continue;
		...
		if (slot === null || entities.some((e) => !e) || metas.some((m) => !m)) {
			if (format === "1v1") {
				...
				const okA = teamA.every(
					(pid) => this.entities.get(pid) && !this.arenaMatches.has(pid),
				);
				...
				if (okB)
					requeue.unshift({
						pids: teamB,
						rating: this.arenaTeamRating(teamB, format),
					});
				if (okA)
					requeue.unshift({
						pids: teamA,
						rating: this.arenaTeamRating(teamA, format),
					});
			}
			return;
		}
	...
	private createFiestaState(): FiestaState {
		return {
			...
			rng: new Rng(
				(this.tickCount * 2654435761 + this.nextArenaMatchId * 40503) >>> 0,
			),
		};
	}
	...
	private placeTeamInArena(
		...
		for (let i = 0; i < pids.length; i++) {
			...
			if (e)
				this.placeInArena(e, origin, spawns[i] ?? spawns[spawns.length - 1]);
		}
	}
	...
	private endArenaMatch(
		...
		const scoreTeam = (team: "A" | "B", delta: number, won: boolean | null) => {
			...
			const enemyNames = enemies
				.map((pid) => this.players.get(pid)?.name ?? "?")
				.join(" & ");
			for (const pid of pids) {
				const meta = this.players.get(pid);
				if (!meta) continue;
				...
				if (ranked) {
					...
					ratingBefore = ratingAfter = this.arenaStanding(
						meta,
						match.format,
					).rating;
				}
		...
		const allPresent = this.arenaAllPids(match).every((pid) =>
			this.entities.get(pid),
		);
		if (!allPresent) {
			this.returnFromArena(match);
			return;
	...
	private mergeAugmentMods(
		base: TalentModifiers,
		augIds: string[],
	): TalentModifiers {
		...
		for (const id of augIds) {
			...
			if (eff.grant)
				m.grants.push({
					ability: eff.grant.ability,
					rank: eff.grant.rank ?? 1,
				});
		}
		return m;
	}
	...
	private fiestaApplyAugments(meta: PlayerMeta, e: Entity): void {
		meta.fiestaMods = this.mergeAugmentMods(
			meta.talentMods,
			meta.fiestaAugments,
		);
		...
		for (const id of meta.fiestaAugments) {
			...
			if (!s) continue;
			if (s.lifestealPct)
				sp.lifestealPct = (sp.lifestealPct ?? 0) + s.lifestealPct;
			if (s.moveSpeedPct)
				sp.moveSpeedPct = (sp.moveSpeedPct ?? 0) + s.moveSpeedPct;
			if (s.scorePerKill)
				sp.scorePerKill = (sp.scorePerKill ?? 0) + s.scorePerKill;
		}
	...
	private fiestaStandardize(meta: PlayerMeta, e: Entity): void {
		...
		meta.talents = defaultBuild(
			meta.cls,
			talentPointsAtLevel(FIESTA_STANDARD_LEVEL),
		);
		meta.talentMods = computeTalentModifiers(meta.cls, meta.talents);
	...
	private fiestaDown(
		...
		const killer =
			killerPid !== null ? (this.entities.get(killerPid) ?? null) : null;
	...
	private fiestaTakedown(
		...
		if (!f.firstBlood) {
			...
			this.emit({ type: "fiestaWord", flavor: "doublekill", pid: killerPid });
		else if (ks >= 3)
			this.emit({ type: "fiestaWord", flavor: "spree", n: ks, pid: killerPid });
	...
	private fiestaOpenWave(match: ArenaMatch): void {
		...
		f.ringTarget = Math.round(
			FIESTA_RING_START - (FIESTA_RING_START - FIESTA_RING_MIN) * frac,
		);
		...
		for (const pid of this.arenaAllPids(match)) {
			...
			const pool = eligibleAugments(
				tier,
				meta.cls,
				this.playerMods(meta).role,
				owned,
			);
	...
	private fiestaRingDamage(match: ArenaMatch): void {
		...
		for (const pid of this.arenaAllPids(match)) {
			...
			const dmg = Math.max(
				1,
				Math.round(e.maxHp * FIESTA_RING_DPS_PCT * interval),
			);
			this.emit({
				type: "damage",
				sourceId: -1,
	...
	private updateFiestaActive(match: ArenaMatch): void {
		...
		if (match.timer >= FIESTA_MAX_DURATION) {
			const winner =
				f.scoreA === f.scoreB ? null : f.scoreA > f.scoreB ? "A" : "B";
			this.endArenaMatch(match, winner, "timeout");
			return;
		}
		// Ease the ring toward its target radius and burn anyone caught outside.
		if (f.ringRadius > f.ringTarget) {
			f.ringRadius = Math.max(
				f.ringTarget,
				f.ringRadius - FIESTA_RING_SHRINK_RATE * DT,
			);
		}
		this.fiestaRingDamage(match);
		this.fiestaUpdatePowerups(match);
		if (f.wave < FIESTA_TOTAL_WAVES && match.timer >= f.nextWaveAt)
			this.fiestaOpenWave(match);
	...
	private fiestaUpdatePowerups(match: ArenaMatch): void {
		...
		for (let i = f.powerups.length - 1; i >= 0; i--) {
			...
			for (const pid of this.arenaAllPids(match)) {
				...
				const e = this.entities.get(pid);
				if (!e || e.dead) continue;
				if (Math.hypot(e.pos.x - p.x, e.pos.z - p.z) > FIESTA_POWERUP_RADIUS)
					continue;
		...
		if (f.powerupTimer <= 0) {
			...
			if (f.powerups.length < FIESTA_POWERUP_MAX)
				this.fiestaSpawnPowerup(match);
		}
	}
		...
		for (let i = 0; i < kit.length; i++) {
			...
			if (e) {
				...
				e.pos = this.groundPos(
					me.pos.x + Math.sin(ang) * 4,
					me.pos.z + Math.cos(ang) * 4,
				);
				e.prevPos = { ...e.pos };
				this.rebucket(e);
	...
	private fiestaPracticeRequeue(includeMe: boolean): void {
		const ids = includeMe
			? [this.primaryId, ...this.fiestaBotPids]
			: [...this.fiestaBotPids];
	...
	updateFiestaBots(): void {
		...
		this.fiestaBotPids = this.fiestaBotPids.filter((pid) =>
			this.entities.has(pid),
		);
	...
	private driveFiestaBot(pid: number): void {
		...
		if (match?.fiesta) {
			...
			if (offer && offer.choices.length)
				this.arenaAugmentPick(this.rng.pick(offer.choices), pid);
		}
	...
	tradeRequest(targetPid: number, pid?: number): void {
		...
		this.tradeInvites.set(targetPid, {
			fromPid: r.meta.entityId,
			expires: this.time + 30,
		});
		this.emit({
	...
	private updateTradesAndInvites(): void {
		...
		for (const session of this.trades.values()) {
			...
			if (
				!ea ||
				!eb ||
				dist2d(ea.pos, eb.pos) > TRADE_RANGE + 4 ||
				ea.dead ||
				eb.dead
			) {
				this.tradeCancel(session.a);
	...
	marketBuy(listingId: number, pid?: number): void {
		...
		if (!listing.house && listing.sellerKey === meta.name) {
			this.error(
				meta.entityId,
				"That is your own listing — cancel it to reclaim it.",
			);
			return;
		}
		...
		if (!listing.house) {
			const proceeds = Math.max(
				0,
				Math.floor(listing.price * (1 - MARKET_CUT)),
			);
	...
	marketInfoFor(pid: number): import("../world_api").MarketInfo | null {
		...
		const matched = filter
			? this.marketListings.filter((l) => {
					...
					return (
						name.includes(filter) || l.itemId.toLowerCase().includes(filter)
					);
				})
			: this.marketListings;
	...
	loadMarket(save: MarketSave | null | undefined): void {
		...
		for (const l of save.listings ?? []) {
			...
			this.marketListings.push({
				...
				expiresAt:
					this.time +
					(Number.isFinite(l.secondsLeft)
						? Math.max(0, l.secondsLeft)
						: MARKET_LISTING_DURATION),
				house: false,
			});
		}
		...
		const maxId = this.marketListings.reduce(
			(m, l) => Math.max(m, l.id + 1),
			1,
		);
		this.nextListingId = Math.max(
			this.nextListingId,
			save.nextListingId ?? 1,
			maxId,
		);
	}
	...
	private updateDoorTriggers(p: Entity): void {
		...
		for (const doorId of this.dungeonDoorIds) {
			...
			if (
				door &&
				door.dungeonId &&
				dist2d(p.pos, door.pos) < DOOR_TRIGGER_RADIUS
			) {
	...
	enterDungeon(dungeonId: string, pid?: number): void {
		...
		if (party?.raid && !raidAllowed) {
			this.error(
				r.meta.entityId,
				"Raid groups cannot enter standard dungeons.",
			);
			return;
		}
		if (!party?.raid && raidRequired) {
			this.error(
				r.meta.entityId,
				"You must convert your party to a raid group first.",
			);
			return;
		}
		if (
			dungeonId === "nythraxis_boss_arena" &&
			!this.canEnterNythraxisRaid(r.meta)
		) {
		...
		if (
			dungeonId === "nythraxis_boss_arena" &&
			this.isRaidLocked(r.meta, dungeonId)
		) {
		...
		if (dungeonId === "nythraxis_boss_arena") {
			const engaged = this.instances.find(
				(i) =>
					...
					i.partyKey === this.instanceKeyFor(r.meta.entityId),
			);
			if (engaged && this.nythraxisInstanceSealed(engaged)) {
				this.error(
					r.meta.entityId,
					"Nythraxis is engaged — the royal door has sealed shut.",
				);
				return;
			}
		}
		const key = this.instanceKeyFor(r.meta.entityId);
		let inst = this.instances.find(
			(i) => i.dungeonId === dungeonId && i.partyKey === key,
		);
		if (!inst) {
			inst = this.instances.find(
				(i) => i.dungeonId === dungeonId && i.partyKey === null,
			);
			if (!inst) {
				this.error(
					r.meta.entityId,
					`All instances of ${dungeon.name} are busy. Try again soon.`,
				);
				return;
			}
		...
		const p = r.e;
		p.pos = this.groundPos(
			origin.x + dungeon.entry.x,
	...
	leaveDungeon(pid?: number): void {
		...
		if (dungeon.id === "nythraxis_boss_arena") {
			const inst = this.instances.find(
				(i) =>
					i.dungeonId === dungeon.id &&
					i.partyKey === this.instanceKeyFor(p.id),
			);
			if (inst && this.nythraxisInstanceSealed(inst)) {
				this.error(
					r.meta.entityId,
					"The royal door is sealed — Nythraxis must fall first.",
				);
				return;
			}
		}
	...
	private claimInstance(inst: InstanceSlot, key: string): void {
		...
		const exit = createGroundObject(
			...
			`${dungeon.name} Exit`,
			this.groundPos(
				origin.x + dungeon.exitOffset.x,
				origin.z + dungeon.exitOffset.z,
			),
		);
	...
	private statsReadout(meta: PlayerMeta, e: Entity): string {
		...
		if (e.resourceType) {
			const res =
				e.resourceType.charAt(0).toUpperCase() + e.resourceType.slice(1);
	...
	private partyReadout(pid: number): string {
		...
		const parts = party.members.map((mPid) => {
			...
			const e = this.entities.get(mPid);
			if (!meta || !e)
				return meta ? `${meta.name} (offline)` : `Player ${mPid} (offline)`;
			const cls = CLASSES[meta.cls].name;
			const state =
				e.hp <= 0 ? "(dead)" : `${Math.round((e.hp / e.maxHp) * 100)}%`;
	...
	private arenaReadout(meta: PlayerMeta): string {
		const part = (
			...
			if (played <= 0)
				return `${label} Rating ${rating} - no matches played yet`;
	...
	private buybackReadout(meta: PlayerMeta): string {
		const slots = meta.vendorBuyback.filter(
	...
	private comboReadout(e: Entity): string {
		...
		const target =
			e.comboTargetId !== null ? this.entities.get(e.comboTargetId) : undefined;
	...
	private dungeonsReadout(): string {
		const parts = DUNGEON_LIST.map(
			(d) =>
				`${d.name} (${zoneAt(d.doorPos.z).name}, ${d.suggestedPlayers} players)`,
		);
	...
	private considerReadout(self: Entity): string {
		const t =
			self.targetId !== null ? this.entities.get(self.targetId) : undefined;
	...
	private poisReadout(self: Entity): string {
		...
		const parts = zone.pois
			.map((p) => ({
				label: p.label,
				d: dist2d(self.pos, { x: p.x, y: 0, z: p.z }),
			}))
			.sort((a, b) => a.d - b.d)
	...
	private listingsReadout(meta: PlayerMeta): string {
		const mine = this.marketListings.filter(
			(l) => !l.house && l.sellerKey === meta.name,
		);
	...
	private overpowerReadout(e: Entity, meta: PlayerMeta): string {
		if (meta.cls !== "warrior")
			return "Overpower is a warrior ability; your class cannot use it.";
	...
	private fallingReadout(e: Entity): string {
		...
		if (e.vy > 0)
			return `You are airborne and rising — ${height}yd above the ground.`;
	...
	private handleChannelMembership(
		...
		if (!arg) {
			this.error(
				pid,
				`Usage: /${action} <channel>. Channels: ${JOINABLE_CHANNELS.join(", ")}.`,
			);
			return;
		}
		if (arg === "general") {
		...
		if (action === "join") {
			...
			this.notice(
				pid,
				`Joined the ${channel} channel. Type /${channel} <message> to talk.`,
			);
		} else {
	...
	private targetReadout(t: Entity): string {
		const kind =
			t.kind === "player" ? "player" : t.ownerId !== null ? "pet" : "mob";
		const health = t.dead
			? "dead"
			: `${Math.round((t.hp / t.maxHp) * 100)}% HP`;
	...
	private xpReadout(meta: PlayerMeta, level: number): string {
		if (level >= MAX_LEVEL)
			return `Level ${MAX_LEVEL} — maximum level reached.`;
	...
	private cooldownsReadout(e: Entity): string {
		...
		const parts = [...e.cooldowns]
			...
			.map(
				([id, remaining]) =>
					`${ABILITIES[id]?.name ?? id} (${Math.ceil(remaining)}s)`,
			);
	...
	private questReadout(meta: PlayerMeta): string {
		...
		for (const [qid, qp] of meta.questLog) {
			...
			const objs = quest.objectives
				.map(
					(o, i) =>
						`${o.label} ${Math.min(qp.counts[i] ?? 0, o.count)}/${o.count}`,
				)
				.join(", ");
	...
	private sessionReadout(meta: PlayerMeta): string {
		...
		const plural = (v: number, word: string) =>
			`${n(v)} ${word}${v === 1 ? "" : "s"}`;
		return (
	...
	private threatReadout(self: Entity): string {
		...
		if (t.kind !== "mob")
			return `Threat is only tracked on enemies; ${t.name} is not one.`;
	...
	private queuedReadout(e: Entity): string {
		if (!e.queuedOnSwing)
			return "You have no ability queued for your next swing.";
	...
	private talentsReadout(meta: PlayerMeta, e: Entity): string {
		...
		const specName = meta.talents.spec
			? (ct.specs.find((s) => s.id === meta.talents.spec)?.name ??
				meta.talents.spec)
			: null;
		const head = specName ?? "no specialization";
		const breakdown = specName
			? `Class ${classPts}, ${specName} ${specPts}`
			: `Class ${classPts}`;
