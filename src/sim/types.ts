...
export type AbilityEffect =
	...
	| {
			...
			judgeMin?: number;
			judgeMax?: number;
	  } // seals / rockbiter: extra damage per swing
...
export function xpToReachLevel(level: number): number {
	return VLEVEL_CUM[
		Math.max(1, Math.min(MAX_VIRTUAL_LEVEL, Math.floor(level)))
	];
}
...
export function xpUntilNextPrestige(
	...
	const target =
		xpToReachLevel(MAX_LEVEL) + (prestigeRank + 1) * PRESTIGE_XP_PER_RANK;
