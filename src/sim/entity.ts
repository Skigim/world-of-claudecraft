...
export function recalcPlayerStats(
	...
	if (mods?.stats.armorPct)
		s.armor = Math.round(s.armor * (1 + mods.stats.armorPct));
	...
	const apFromStats =
		...
		cls === "druid"
			...
			: cls === "rogue" || cls === "hunter"
				? s.str + s.agi
				: s.str;
	e.attackPower = Math.round(
		(apFromStats + bonusAp) * (1 + (mods?.stats.apPct ?? 0)),
	);
	// Hunters: ranged AP = 2/agi (vanilla)
	e.rangedPower =
		cls === "hunter"
			? Math.round((s.agi * 2 + bonusAp) * (1 + (mods?.stats.apPct ?? 0)))
			: 0;
	...
	if (mods?.stats.maxHpPct)
		e.maxHp = Math.round(e.maxHp * (1 + mods.stats.maxHpPct));
	...
	const formResource: "rage" | "energy" | null = bearForm
		? "rage"
		: catForm
			? "energy"
			: null;
	if (formResource) {
		...
		if (e.resourceType !== formResource)
			e.resource = formResource === "energy" ? 100 : 0;
...
export function createMob(
	...
	e.maxHp = Math.round(
		(template.hpBase + template.hpPerLevel * (level - 1)) * hpMult,
	);
	e.hp = e.maxHp;
...
export function createGroundObject(
	id: number,
	itemId: string,
	name: string,
	pos: Vec3,
