...
export const NATIVE_API_ORIGIN = normalizeOrigin(
...
export class Api {
	...
	async realmStatus(
		url: string,
	): Promise<{ online: boolean; players: number }> {
		try {
			const res = await fetch(apiUrl("/api/status", url), {
	...
	async uploadCard(
		...
		const data = await res.json().catch(() => ({}));
		if (!res.ok)
			throw new Error(data.error ?? `card upload failed (${res.status})`);
		return { url: data.url };
	}
...
export class ClientWorld implements IWorld {
	...
	constructor(
		...
		this.ws.onopen = () => {
			this.ws.send(
				JSON.stringify(
					buildWebSocketAuthMessage(token, characterId, this.clientSeed),
				),
			);
		};
	...
	private inputSignature(): string {
		...
		const facing =
			this.mouselookFacing === null
				? ""
				: Math.round(this.mouselookFacing * 10000).toString();
		return [
			mi.forward ? 1 : 0,
			mi.back ? 1 : 0,
			mi.turnLeft ? 1 : 0,
	...
	private applySnapshot(snap: any): void {
		...
		const contAlpha =
			this.lastSnapAt > 0
				? Math.min(
						1.25,
						(now - this.lastSnapAt) / Math.max(20, this.snapInterval),
					)
				: 1;
		if (this.lastSnapAt > 0) {
			const gap = now - this.lastSnapAt;
			if (gap > 5 && gap < 500)
				this.snapInterval = this.snapInterval * 0.9 + gap * 0.1;
		}
		this.lastSnapAt = now;
		...
		const applyWire = (w: any): Entity | null => {
			...
			if (prevUpdatedAt !== undefined) {
				...
				if (gap > 5 && gap < 450) {
					e.netInterval =
						prevInterval === undefined ? gap : prevInterval * 0.7 + gap * 0.3;
				}
			}
			e.netUpdatedAt = now;
			...
			if (
				(wasDead && !nowDead) ||
				teleDx * teleDx + teleDz * teleDz > TELEPORT_SNAP_DIST_SQ
			) {
		...
		if (s && e) {
			...
			if (s.cds !== undefined)
				e.cooldowns = new Map(
					Object.entries(s.cds).map(([k, v]) => [k, Number(v)]),
				);
			...
			if (s.qlog !== undefined)
				this.questLog = new Map(
					(s.qlog as QuestProgress[]).map((q) => [q.questId, q]),
				);
			if (s.qdone !== undefined) this.questsDone = new Set(s.qdone);
			if (s.qlog !== undefined || s.qdone !== undefined)
			...
			if (s.tal !== undefined && s.tal) {
				...
				this.activeLoadout =
					typeof s.tal.activeLoadout === "number" ? s.tal.activeLoadout : -1;
			}
	...
	private deadTargetCast(def: ResolvedAbility["def"] | undefined): boolean {
		if (!def || !def.requiresTarget || def.targetType === "friendly")
			return false;
	...
	castAbility(abilityId: string): void {
		if (
			this.deadTargetCast(this.known.find((k) => k.def.id === abilityId)?.def)
		) {
	...
	async searchCharacters(query: string): Promise<CharacterSearchResult[]> {
		...
		try {
			const res = await fetch(
				apiUrl(`/api/search?q=${encodeURIComponent(q)}`, this.base),
	...
	async leaderboard(): Promise<LeaderboardEntry[]> {
		try {
			const res = await fetch(
				apiUrl("/api/leaderboard?metric=lifetimeXp&limit=100", this.base),
			);
			if (!res.ok) return [];
	...
	talentPoints(): { total: number; spent: number } {
		...
		return {
			total: talentPointsAtLevel(level),
			spent: pointsSpent(this.talents),
		};
	}
		...
		if (wasActive) {
			this.activeLoadout =
				this.loadouts.length > 0
					? Math.min(index, this.loadouts.length - 1)
					: -1;
			const next =
				this.activeLoadout >= 0 ? this.loadouts[this.activeLoadout] : null;
			if (next) {
