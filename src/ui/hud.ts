...
const SFX_CAST_SCHOOLS = new Set([
	"fire",
	"frost",
	"arcane",
	"shadow",
...
function mobVoiceFamily(templateId: string): string | null {
	if (templateId === "wild_boar" || templateId === "elder_bristleback")
		return "boar";
	const fam = MOBS[templateId]?.type;
...
function materialImpactKey(tgt: Entity): string {
	if (tgt.kind === "player")
		return tgt.templateId === "warrior" || tgt.templateId === "paladin"
			...
			: "impact_leather";
	if (tgt.kind === "mob")
		return MOBS[tgt.templateId]?.type === "undead"
			? "impact_bone"
			: "impact_flesh";
	return "impact_flesh";
}
...
export class Hud {
	...
	private static readonly FORM_TOGGLE_IDS = new Set([
		"bear_form",
		"cat_form",
		"travel_form",
	]); // shift toggles, castable in any form
	...
	private chatClock: ChatClock = clampChatClock(
		localStorage.getItem("chatClock"),
	);
	...
	private castbarLabelEl = this.castbarEl.querySelector(
		".label",
	) as HTMLElement;
	private castbarTimerEl = this.castbarEl.querySelector(
		".timer",
	) as HTMLElement;
	private targetCastbarEl = $("#tf-castbar");
	private targetCastbarFillEl = this.targetCastbarEl.querySelector(
		".fill",
	) as HTMLElement;
	private targetCastbarLabelEl = this.targetCastbarEl.querySelector(
		".label",
	) as HTMLElement;
	private targetCastbarTimerEl = this.targetCastbarEl.querySelector(
		".timer",
	) as HTMLElement;
	...
	private activeLootRolls = new Map<
		number,
		{
			event: Extract<SimEvent, { type: "lootRoll" }>;
			receivedAt: number;
			durationMs: number;
		}
	>();
	...
	private stagedTrade: { items: InvSlot[]; copper: number } = {
		items: [],
		copper: 0,
	};
	...
	private mapDrag: { px: number; py: number; cx: number; cz: number } | null =
		null;
	private mapView: {
		spanX: number;
		spanZ: number;
	...
	constructor(
		...
		window.addEventListener("pointermove", (ev) => {
			if (this.emoteWheelOpen)
				this.updateEmoteWheelPointer(ev.clientX, ev.clientY);
		});
		...
		document.addEventListener("pointerdown", (ev) => {
			const target = ev.target as Node | null;
			if (!target) return;
			const communityMenu = document.getElementById(
				"community-menu",
		...
		document
			.getElementById("mobile-more-close")
		...
		$("#quest-tracker").addEventListener("click", (e) => {
			if ((e.target as HTMLElement).closest(".qt-header"))
		...
		$("#mm-leaderboard").addEventListener("click", () =>
	...
	perfStats(): {
		...
		return {
			...
			hotDomSkipRate:
				total > 0
					? Math.round((this.hotDomSkippedWrites / total) * 1000) / 1000
					: 0,
		};
	}
	...
	private initWindowManagement(): void {
			...
			for (const m of mutations) {
				if (m.type === "childList") {
					m.addedNodes.forEach((node) => {
						...
						node
							.querySelectorAll<HTMLElement>(".window.panel")
							.forEach(observeWindow);
					});
					continue;
				}
				if (
					m.target instanceof HTMLElement &&
					m.target.matches(".window.panel")
				) {
		...
		document.addEventListener("pointerdown", (ev) => {
			...
			if (ev.button !== 0 || !target || !this.isWindowDragHandle(target, el))
				return;
			ev.preventDefault();
			this.hideTooltip();
	...
	private windowZValue(el: HTMLElement): number {
		const z = Number.parseInt(
			el.style.zIndex || getComputedStyle(el).zIndex || "",
			10,
		);
	...
	private initChatTabs(): void {
		...
		for (const ch of this.chatTabs)
			if (channelNeedsJoin(ch)) this.sim.chat(`/join ${ch}`);
		this.renderChatTabs();
	...
	private initChatBoxGeometry(): void {
		...
		tabs.addEventListener("pointerdown", (ev) =>
			this.onChatBoxMoveStart(ev, wrap, tabs),
		);
		grip.addEventListener("pointerdown", (ev) =>
			this.onChatBoxResizeStart(ev, wrap, frame),
		);
		document.addEventListener("pointermove", (ev) =>
			this.onChatBoxPointerMove(ev),
		);
	...
	private ensureChatBoxGeometry(wrap: HTMLElement, tabs: HTMLElement): void {
		...
		this.chatBox = {
			...
			height: frameRect
				? frameRect.height
				: Math.max(0, wrapRect.height - chromeH),
		};
	}

	private onChatBoxMoveStart(
		ev: PointerEvent,
		wrap: HTMLElement,
		tabs: HTMLElement,
	): void {
	...
	syncChatTabsForInput(typed: string): void {
		...
		if (m[1].toLowerCase() === "join")
			this.addChatTab(channel, { join: false });
	...
	activeChatPlaceholder(): string {
		const ch = this.chatFilterChannel();
		return ch
			? t("hud.core.chatChannels.sendingTo", {
					channel: t(CHANNEL_LABEL_KEYS[ch]),
				})
	...
	private saveEmoteWheelSlots(): void {
		try {
			localStorage.setItem(
				this.emoteWheelKey(),
				JSON.stringify(this.emoteWheelSlots),
			);
			localStorage.setItem(this.emoteWheelVersionKey(), "1");
		} catch {
			/* storage unavailable */
	...
	private showEmoteWheel(pinned = false): void {
		...
		const slots = this.emoteWheelSlots
			.filter(isOverheadEmoteId)
			.slice(0, EMOTE_WHEEL_LIMIT);
		...
		slots.forEach((id, i) => {
			...
			const angle =
				-Math.PI / 2 + (i / Math.max(1, slots.length)) * Math.PI * 2;
			btn.style.left = `${50 + Math.cos(angle) * 39}%`;
			btn.style.top = `${50 + Math.sin(angle) * 39}%`;
			el.appendChild(btn);
		});
		el.querySelector<HTMLButtonElement>(".emote-wheel-edit")?.addEventListener(
			"click",
			(ev) => {
				ev.preventDefault();
				ev.stopPropagation();
	...
	private updateEmoteWheelPointer(x: number, y: number): void {
		...
		if (dist <= 44) {
			...
			dist >= 58 &&
			dist <= rect.width * 0.58 &&
			this.emoteWheelSlots.length > 0
		) {
			const angle =
		...
		el.querySelector(".emote-wheel-edit")?.classList.toggle(
			"selected",
			hover === "edit",
		);
	...
	private renderEmoteEditor(): void {
		...
		const syncButtons = () => {
			grid
				.querySelectorAll<HTMLButtonElement>(".emote-editor-item")
				.forEach((b) => {
	...
	moneyHtml(copper: number): string {
		...
		if (parts.silver > 0 || parts.gold > 0)
			html += coin(parts.silver, "s", "itemUi.money.silver");
	...
	private refreshLocalizedDynamicUi(): void {
		...
		if (
			...
			$("#vendor-window").style.display === "block"
		)
			this.renderVendor();
		if (this.marketOpen) {
			this.lastMarketSig = "";
		...
		if (dialog.style.display !== "block" || this.openGossipNpcId === null)
			return;
	...
	private abilityTooltip(res: ResolvedAbility): string {
		...
		if (costLine.length)
			html += `<div class="tt-stat">${costLine.map(esc).join(" &nbsp; ")}</div>`;
		const castLine = [abilityCastLine(res)];
		if (a.cooldown > 0)
			castLine.push(
				t("abilityUi.tooltip.cooldownSeconds", {
					seconds: formatAbilityNumber(a.cooldown),
				}),
			);
		...
		if (requirements.length) {
			html += requirements
				.map((line) => `<div class="tt-sub">${esc(line)}</div>`)
				.join("");
		}
		return html;
	}
	...
	private playerHotbarForm(): HotbarForm {
		if (this.sim.cfg.playerClass === "druid") {
			if (this.sim.player.auras.some((a) => a.kind === "form_bear"))
				return "bear";
	...
	private shouldAutoPlaceOnForm(id: string, form: HotbarForm): boolean {
		if (form === "bear" || form === "cat") {
			return (
				ABILITIES[id]?.requiresForm === form || Hud.FORM_TOGGLE_IDS.has(id)
			);
		}
	...
	private formKitAbilityIds(form: HotbarForm): string[] {
		return this.sim.known
			.map((k) => k.def.id)
			.filter((id) => this.shouldAutoPlaceOnForm(id, form));
	}
	...
	private isFormKitBar(form: HotbarForm = this.activeHotbarForm): boolean {
		return (
			this.sim.cfg.playerClass === "druid" &&
			(form === "bear" || form === "cat")
		);
	}
	...
	private seedFormBarIfNeeded(parsed: HotbarAction[]): boolean {
		...
		try {
			normalRaw = JSON.parse(
				localStorage.getItem(this.slotMapKey("normal")) ?? "null",
			);
		} catch {
			/* corrupt */
		}
	...
	private loadSlotMap(): void {
		...
		const emptyFormMap =
			...
			parsed.every((action) => action === null);
		if (emptyFormMap) {
	...
	private saveSlotMap(): void {
		try {
			localStorage.setItem(
				this.slotMapKey(),
				JSON.stringify(this.hotbarActions),
			);
		} catch {
			/* storage unavailable */
		}
	...
	private addAbilityToHotbar(abilityId: string): boolean {
		...
		this.hotbarActions = placeAbilityOnSlot(
			this.hotbarActions,
			abilityId,
			target,
		);
	...
	private resetActiveFormBarToDefault(): void {
		...
		this.knownAbilityIdsAtLastSlotSync = new Set(
			this.sim.known.map((k) => k.def.id),
		);
		this.markFormBarSeeded();
		this.saveSlotMap();
	...
	private syncSlotMap(): void {
		...
		const consider = (id: string) => {
			if (this.shouldAutoPlaceOnForm(id, this.activeHotbarForm))
				autoPlaceAbilityIds.add(id);
		};
		...
		if (formToggle && knownAbilityIds.includes(formToggle))
			autoPlaceAbilityIds.add(formToggle);
		...
		for (let i = 0; i < 12; i++) {
			...
			if (slot >= 1) {
				...
				btn.addEventListener("dragover", (e) => {
					const dragged =
						this.dragAction?.action ?? this.readDraggedAction(e.dataTransfer);
					if (!dragged) return;
					if (this.dragAction?.sourceIndex === slot - 1) return;
					e.preventDefault(); // required to permit the drop
					e.dataTransfer!.dropEffect =
						this.dragAction?.sourceIndex === null && dragged.type === "item"
							? "copy"
							: "move";
				...
				btn.addEventListener("drop", (e) => {
					...
					if (dragged.sourceIndex !== null)
						...
						this.hotbarActions = placeItemOnSlot(
							this.hotbarActions,
							action.id,
							slot - 1,
						);
	...
	private clearMobileHotbarDrag(): void {
		...
		document
			.querySelectorAll("#actionbar .mobile-drag-source")
			.forEach((el) => {
	...
	private bindMobileActionDrag(btn: HTMLButtonElement, slot: number): void {
		...
		btn.addEventListener("pointermove", (e) => {
			...
			this.clearActionDropTargets();
			const targetBtn =
				targetSlot !== null ? this.abilityButtons[targetSlot]?.btn : null;
			if (targetBtn) targetBtn.classList.add("drop-target");
			this.abilityButtons[drag.sourceIndex + 1]?.btn.classList.add(
				"mobile-drag-source",
			);
		});
	...
	private refreshKeybindLabels(): void {
		for (let i = 0; i < this.abilityButtons.length; i++) {
			this.abilityButtons[i].keybindEl.textContent = this.keybinds.primaryLabel(
				`slot${i}`,
			);
		}
		const sideButtons: [
	...
	private renderPetBar(): void {
		...
		addButton(
			...
			petModeLabel(mode),
			petTooltip(
				`${t("hud.pet.stanceTitle")}: ${petModeLabel(mode)}`,
				t("hud.pet.stanceDesc"),
			),
			() => {
	...
	update(): void {
		...
		const talGlow =
			talentsFor(sim.cfg.playerClass) !== null && tp.spent < tp.total;
		document
			.getElementById("mm-talents")
			?.classList.toggle("has-points", talGlow);
		document
			.getElementById("mobile-talents")
			?.classList.toggle("has-points", talGlow);
		...
		this.setText(
			this.pfResTextEl,
			`${Math.round(p.resource)} / ${p.maxResource}`,
		);
		const resClass =
			"bar " +
			(p.resourceType === "rage"
				...
				: p.resourceType === "energy"
					? "energy"
					: "mana");
		if (this.pfResourceEl.className !== resClass)
			this.pfResourceEl.className = resClass;
		...
		if (target && target.kind !== "object") {
			...
			this.targetFrameEl.classList.toggle(
				"elite",
			...
			this.setText(
				this.targetLevelEl,
				MOBS[target.templateId]?.boss ? "☠" : String(target.level),
			);
			this.setTransform(
				this.targetHpEl,
				`scaleX(${target.hp / Math.max(1, target.maxHp)})`,
			);
			...
			const targetNameColor = target.hostile
				? "var(--color-hostile)"
				: "var(--color-friendly)";
			...
			if (this.lastPortraitTarget !== target.id) {
				...
				if (target.kind === "player") {
					...
					this.portraits.drawCrest(
						this.targetPortraitEl,
						crestIdForEntity(target.kind, MOBS[target.templateId]?.type),
					);
				}
			}
			...
			if (tcb.visible) {
				...
				this.setWidth(
					this.targetCastbarFillEl,
					`${(tcb.fill * 100).toFixed(1)}%`,
				);
		...
		if (p.castingAbility) {
			...
			this.setWidth(
				this.castbarFillEl,
				`${((c.remaining / CONSUME_DURATION) * 100).toFixed(1)}%`,
			);
			this.setText(
				this.castbarLabelEl,
				p.eating && p.drinking
		...
		if (p.autoAttack && target && !target.dead && target.kind !== "object") {
			...
			(sw.querySelector(".fill") as HTMLElement).style.width =
				`${(frac * 100).toFixed(1)}%`;
		...
		if ($("#spellbook").style.display === "block")
			this.refreshSpellbookHotbarControls();
		for (let i = 0; i < this.abilityButtons.length; i++) {
			...
			const cdHeight =
				shown > 0
					? `${Math.min(100, (shown / Math.max(0.01, denom)) * 100)}%`
					: "0%";
			if (ab.cdOverlay.style.height !== cdHeight)
				ab.cdOverlay.style.height = cdHeight;
			this.setText(ab.cdText, cd > 1 ? Math.ceil(cd).toString() : "");
			ab.btn.classList.toggle("unusable", p.resource < known!.cost);
			const oor =
				a.requiresTarget &&
				tgtDist !== null &&
				tgtDist > (a.range > 0 ? a.range : MELEE_RANGE);
		...
		const showOverflow =
			(this.optionsHooks?.settings.get("showOverflowXp") ?? 1) >= 0.5;
		const bar = xpBarView({
			level: p.level,
			xp: sim.xp,
		...
		if (mediumHud) {
			...
			if (!inDungeon && currentZone.id !== this.lastZoneId) {
				...
				if (pastDeadBand) {
					if (this.lastZoneId !== "") {
						...
						this.log(
							t("hud.core.enteringZone", { zone: currentZoneName }),
							"#ffd100",
						);
			...
			if (subzone !== this.lastSubzone) {
				...
				if (subzone) {
					const poiIndex = currentZone.pois.findIndex(
						(q) => q.label === subzone,
					);
					this.showSubzone(
						poiIndex >= 0 ? zonePoiLabel(currentZone.id, poiIndex) : subzone,
					);
				}
			}
			...
			const musicDungeonId =
				inDungeon || inNythraxisArena ? (dungeon?.id ?? null) : null;
			if (
				shouldResetMusicForDungeonEntry(this.lastMusicDungeonId, musicDungeonId)
			) {
		...
		if (
			inArenaMatch &&
			!this.arenaMatchSeen &&
			$("#arena-window").style.display === "block"
		) {
	...
	private updateAbsorb(sel: string, e: Entity | null): void {
		...
		const v = e
			? absorbBarView(e)
			: { fillFrac: 0, overshield: false, total: 0 };
	...
	private updateLowResource(p: Entity): void {
		...
		const sig = v.active
			? `${v.opacity.toFixed(2)}|${v.pulseSeconds.toFixed(2)}|${v.label}`
			: "";
	...
	private toggleQuestTrackerCollapsed(): void {
		...
		settings.set(
			...
			!settings.get("questTrackerCollapsed"),
		);
		audio.click();
		this.updateQuestTracker();
		if (refocus)
			(
				$("#quest-tracker").querySelector(".qt-header") as HTMLElement | null
			)?.focus();
	}
	...
	private scheduleMapPrewarm(): void {
		...
		if (w.requestIdleCallback) {
			...
			this.mapPrewarmHandle = window.setTimeout(
				() => this.pumpMapPrewarm(),
				16,
			);
	...
	private updateCompass(): void {
		...
		if (this.compassHeadingEl && view.heading !== this.lastCompassHeading) {
			...
			this.compassHeadingEl.textContent = t(
				`hudChrome.compass.${view.heading}`,
			);
		}
	}
	...
	private updateMinimap(): void {
		...
		const guildNames = social?.guild
			? new Set(social.guild.members.map((m) => m.name))
			: null;
		...
		for (const e of this.sim.entities.values()) {
			...
			if (e.kind === "player" && !(partyPids && partyPids.has(e.id))) {
				...
				const hasAvail = e.questIds.some(
					(q) =>
						QUESTS[q].giverNpcId === e.templateId &&
						this.sim.questState(q) === "available",
				);
				const hasReady = e.questIds.some(
					(q) =>
						isQuestTurnInNpc(QUESTS[q], e.templateId) &&
						this.sim.questState(q) === "ready",
				);
	...
	private renderArenaWindow(): void {
		...
		const ladder =
			ladderRows
				...
				.join("") ||
			`<div class="ladder-empty">${esc(t("hud.arena.noChallengers"))}</div>`;

		const bracketLabel = (fmt: ArenaFormat) =>
			fmt === "fiesta" ? t("fiesta.bracket") : fmt;
		...
		el.querySelector('[data-act="queue"]:not([disabled])')?.addEventListener(
			"click",
			() => {
	...
	private updateArenaStatus(): void {
		...
		if (m.format === "2v2") {
			...
			const cls = CLASSES[m.oppClass]
				? classDisplayName(m.oppClass)
				: m.oppClass;
	...
	private updateMapWindow(): void {
		...
		const cx = Math.max(
			full.minX + spanX / 2,
			Math.min(full.maxX - spanX / 2, baseX),
		);
		const cz = Math.max(
			full.minZ + spanZ / 2,
			Math.min(full.maxZ - spanZ / 2, baseZ),
		);
		const region = {
			minX: cx - spanX / 2,
			maxX: cx + spanX / 2,
		...
		if (!this.mapDrag)
			canvas.style.cursor = this.mapZoom > 1 ? "grab" : "default";
		...
		for (const dungeon of DUNGEON_LIST) {
			if (dungeon.doorPos.z < zone.zMin || dungeon.doorPos.z >= zone.zMax)
				continue;
		...
		if (social) {
			...
			if (social.guild)
				for (const m of social.guild.members) plotAlly(m, "#60a5fa");
		}
	}
	...
	private drawMapDetail(
		...
		const ppu =
			((toMap(region.minX + 1, region.minZ).mx -
				toMap(region.minX, region.minZ).mx) **
				2) **
			0.5;

		// vegetation: pine/oak/rock scattered across the world (cached once)
		if (!this.mapDecorations)
			this.mapDecorations = generateDecorations(this.sim.cfg.seed);
		...
		for (const [x, z] of PROPS.campfires)
			dot(x, z, "#ff9a3c", Math.max(1.4, ppu * 0.5));
	}
	...
	private reconcileSfx(): void {
		...
		if (this.mobAggroed.size) {
			for (const id of this.mobAggroed)
				if (!sim.entities.has(id)) this.mobAggroed.delete(id);
		}
		...
		switch (ev.type) {
			...
			case "spellfx": {
				if (ev.fx === "projectile") {
					...
					if (s)
						this.combat(`proj_${ev.school}`, s.pos.x, s.pos.y, s.pos.z, 0.55);
				} else if (ev.fx === "nova") {
					const e =
						sim.entities.get(ev.sourceId) ?? sim.entities.get(ev.targetId);
			...
			case "heal2": {
				const tgt = sim.entities.get(ev.targetId);
				if (tgt)
					this.combat("heal_impact", tgt.pos.x, tgt.pos.y, tgt.pos.z, 0.45, {
						cooldown: 0.1,
					});
				return;
			}
			case "aura": {
				...
				this.combat(
					ev.gained ? "buff_apply" : "debuff_apply",
					p.x,
					p.y,
					p.z,
					0.4,
	...
	handleEvents(events: SimEvent[]): void {
		...
		for (const ev of events) {
			...
			if (this.isNythraxisEvent(ev))
				this.lastNythraxisCombatEventAt = performance.now();
			switch (ev.type) {
				case "damage": {
					const src = sim.entities.get(ev.sourceId);
					const tgt = sim.entities.get(ev.targetId);
					...
					if (ev.kind === "miss" || ev.kind === "dodge") {
						...
						if (isPlayerSource) {
							this.combatLog(
								t(
									ev.kind === "miss" ? "hud.combat.miss" : "hud.combat.dodged",
									{
					...
					if (isPlayerSource && !isPlayerTarget) {
						...
						this.combatLog(
							t(
								ev.crit ? "hud.combat.damageDoneCrit" : "hud.combat.damageDone",
								{
									ability: combatAbilityName(ev.ability),
									target: entityDisplayName(tgt),
									amount: ev.amount,
				...
				case "heal": {
					if (ev.amount > 0) {
						const healed =
							ev.targetId === sim.playerId
								? sim.player
								: sim.entities.get(ev.targetId);
				...
				case "death": {
					...
					if (e && ev.entityId !== sim.playerId)
						this.combatLog(
							t("hud.combat.death", { name: entityDisplayName(e) }),
							"#aaa",
						);
					break;
				}
				case "xp": {
					this.fct(
						sim.player,
						t("hud.core.xpFloat", { amount: ev.amount }),
						"#b974ff",
						false,
					);
				...
				case "levelup": {
					...
					if (
						ev.level === FIRST_TALENT_LEVEL &&
						talentsFor(this.sim.cfg.playerClass)
					) {
				...
				case "virtualLevelUp": {
					...
					this.showBanner(
						`${t("game.progression.virtualLevelUp")} ${ev.level}!`,
					);
					this.log(
						`${t("game.progression.virtualLevelUp")} ${ev.level}!`,
						"#ffd100",
				...
				case "skinEvent":
					this.openSkinEvent(
						ev.rank,
						ev.catalog === "mech" ? { mech: true } : undefined,
					);
					break;
				case "error":
				...
				case "questProgress":
					this.log(
						this.localizeQuestProgressText(ev.questId, ev.text),
						"#dcd29f",
					);
					this.refreshGossip();
					break;
				...
				case "chat": {
					...
					if (
						...
						const bubble =
							ev.channel === "emote" ? `${ev.from} ${masked}` : masked;
						this.renderer.showChatBubble(
							ev.entityId,
							bubble,
							ev.channel === "yell",
						);
				...
				case "duelCountdown":
					this.showBanner(
						t("hud.system.duelCountdown", { seconds: ev.seconds }),
					);
				...
				case "arenaFound": {
					const name =
						ev.enemies.length > 1
							? ev.enemies.map((e) => e.name).join(" & ")
							: ev.oppName;
					const cls = CLASSES[ev.oppClass]
						? classDisplayName(ev.oppClass)
						: ev.oppClass;
	...
	private logZoneWelcome(zone: ZoneDef): void {
		if (
			zone.welcomeQuestId &&
			this.sim.questState(zone.welcomeQuestId) !== "available"
		)
			return;
	...
	private chatLogFrom(
		...
		for (const part of rendered.split(
			/(__WOC_CHAT_NAME__|__WOC_CHAT_MESSAGE__)/,
		)) {
			if (part === nameToken) {
				div.append(sender);
				senderAppended = true;
	...
	private maskChat(text: string): string {
		...
		if (!(this.optionsHooks?.settings.get("filterProfanity") ?? true))
			return text;
	...
	private localizeErrorText(text: string): string {
		...
		match = /^That ability requires the target below (\d+)% health\.$/.exec(
			text,
		);
		...
		match = /^Several players match '(.+)'\. Use exact capitalization\.$/.exec(
			text,
		);
		...
		match =
			/^Chat locked for (\d+)s because you are sending messages too quickly\.$/.exec(
				text,
			);
		...
		match = /^You may keep at most (\d+) goods on the market at once\.$/.exec(
			text,
		);
		if (match)
		...
		match =
			/^That is your own listing (?:\u2014|-) cancel it to reclaim it\.$/.exec(
				text,
			);
	...
	private localizeSystemText(text: string): string {
		...
		if (key) return t(key);
		for (const dungeon of DUNGEON_LIST) {
			if (text === dungeon.enterText)
				return dungeonText(dungeon.id, "enterText");
			if (text === dungeon.leaveText)
				return dungeonText(dungeon.id, "leaveText");
		}
		...
		match =
			/^Your market listing of (.+) expired and waits at the Merchant\.$/.exec(
				text,
			);
		if (match)
		...
		match =
			/^(.+) is meant for a full party of (\d+)\. Tread carefully\.$/.exec(
				text,
			);
		if (match) {
	...
	private localizeQuestProgressText(questId: string, text: string): string {
		...
		const objectiveIndex = quest.objectives.findIndex(
			(objective) => objective.label === match[1],
		);
		const label =
			objectiveIndex >= 0
				? questObjectiveLabel(questId, objectiveIndex)
				: match[1];
	...
	private localizeLootText(text: string): string {
		...
		if (match)
			return t("itemUi.lootRoll.everyonePassed", {
				item: itemDisplayNameFromSource(match[1]),
			});
		...
		match =
			/^(.+) bought your (.+) for (.+?) (?:\u2014|-) collect (.+) from the Merchant\.$/.exec(
				text,
			);
		if (match)
	...
	private renderFiestaScore(f: import("../world_api").FiestaMatchInfo): void {
		...
		const scored =
			...
			(this.fiestaScoreSeen.a !== f.scoreA ||
				this.fiestaScoreSeen.b !== f.scoreB);
		const myPrev =
			f.team === "A" ? this.fiestaScoreSeen.a : this.fiestaScoreSeen.b;
		const theirPrev =
			f.team === "A" ? this.fiestaScoreSeen.b : this.fiestaScoreSeen.a;
		el.dataset.sig = sig;
		el.innerHTML = `
	...
	private renderFiestaAugments(
		offer: import("../world_api").FiestaAugmentOffer,
	): void {
	...
	private augmentCategorySvg(cat: AugmentCategory): string {
		const p: Record<AugmentCategory, string> = {
			offense:
				'<path d="M3 21l6-6m0 0l9-9 2 2-9 9m-2-2l-2 2 2 2 2-2m-2-2l2 2"/>', // sword
			...
			case "spree":
				return {
					text: t("fiesta.word.spree", {
						n: formatNumber(n ?? 3, { maximumFractionDigits: 0 }),
					}),
					tier: 2,
					color: "#ff7a1a",
				};
	...
	private renderGossip(npc: Entity): void {
		...
		const discussionQuests = [...this.sim.questLog.values()]
			.filter(
				(qp) => qp.state === "active" && npc.questIds.includes(qp.questId),
			)
			.filter((qp) =>
		...
		const npcName = def
			? npcDisplayName(npc.templateId)
			: mobDisplayName(npc.templateId);
	...
	private renderQuestDetail(npc: Entity, questId: string): void {
		...
		voice.play(
			state === "ready"
				? `quest__${questId}__complete`
				: `quest__${questId}__offer`,
		);
		el.setAttribute("role", "dialog");
		el.setAttribute("aria-modal", "false");
		el.setAttribute("aria-labelledby", "quest-dialog-title");
	...
	private refreshGossip(): void {
		if (
			this.openGossipNpcId === null ||
			$("#quest-dialog").style.display !== "block"
		)
			return;
		const npc = this.sim.entities.get(this.openGossipNpcId);
	...
	private updateLootRollTimers(now: number): void {
		...
		for (const row of root.querySelectorAll<HTMLElement>(".loot-roll")) {
			...
			const remaining = Math.max(
				0,
				1 - (now - roll.receivedAt) / roll.durationMs,
			);
	...
	private renderLootRolls(): void {
        ...
        <div class="loot-roll-actions">
          ...
          <button type="button" class="loot-roll-btn pass" data-choice="pass">${esc(t("itemUi.lootRoll.pass"))}</button>
			...
			if (item)
				this.attachTooltip(
					row.querySelector(".loot-roll-item") as HTMLElement,
					() => this.itemTooltip(item),
				);
			row
				...
				.forEach((btn) => {
					...
					btn.setAttribute(
						"aria-label",
						t(`itemUi.lootRoll.${choice}Aria`, { item: itemName }),
					);
					btn.addEventListener("click", () =>
						this.submitLootRoll(rollId, choice),
					);
				});
	...
	private renderVendor(): void {
		...
		const html = `<div class="panel-title"><span>${esc(t("itemUi.vendor.goodsTitle", { name: entityDisplayName(npc) }))}</span><button type="button" class="x-btn" data-close aria-label="${esc(t("itemUi.vendor.close"))}">${svgIcon("close")}</button></div>`;
		el.innerHTML = html;
		for (const itemId of npc.vendorItems) {
			...
			row.setAttribute(
				"aria-label",
				t("itemUi.vendor.buyAria", { item: itemName, price }),
			);
			...
			this.attachTooltip(
				...
				() =>
					this.itemTooltip(item) +
					`<div class="tt-sub">${esc(t("itemUi.tooltip.clickBuy"))}</div>`,
			);
			el.appendChild(row);
		}
		...
		const buyback = this.sim.vendorBuyback.filter(
			(s) => ITEMS[s.itemId] && s.count > 0,
		);
		if (buyback.length === 0) {
		...
		for (const s of buyback) {
			...
			row.setAttribute(
				"aria-label",
				t("itemUi.vendor.buybackAria", { item: itemName, price }),
			);
			...
			this.attachTooltip(
				...
				() =>
					this.itemTooltip(item) +
					`<div class="tt-sub">${esc(t("itemUi.tooltip.clickBuyback"))}</div>`,
			);
			el.appendChild(row);
		}
	...
	private marketSubtypeOptions(): readonly MarketSubtypeFilter[] {
		...
		if (this.marketItemTypeFilter === "weapon")
			return MARKET_WEAPON_TYPE_FILTERS;
		return ["all"];
	}
	...
	private marketSubtypeOptionLabel(filter: MarketSubtypeFilter): string {
		...
		if (this.marketItemTypeFilter === "armor")
			return itemSlotName(filter as EquipSlot);
	...
	private renderMarketFilters(): string {
		...
		const hasSubtype =
			this.marketItemTypeFilter === "armor" ||
			this.marketItemTypeFilter === "weapon";
		return (
			...
			this.renderMarketFilterMenu(
				...
				MARKET_ITEM_TYPE_FILTERS,
				(filter) =>
					t(this.marketItemTypeLabelKey(filter as MarketItemTypeFilter)),
			) +
			(hasSubtype
				? this.renderMarketFilterMenu(
						"subtype",
						...
						(filter) =>
							this.marketSubtypeOptionLabel(filter as MarketSubtypeFilter),
					)
				: "") +
			this.renderMarketFilterMenu(
				"rarity",
	...
	private renderMarket(): void {
		...
		const collectN = info
			? (info.collectionCopper > 0 ? 1 : 0) + info.collectionItems.length
			: 0;
		...
		el.querySelectorAll<HTMLButtonElement>(".mkt-select-btn").forEach(
			(button) => {
	...
	private refreshMarket(): void {
		...
		const collectTab = $("#market-window").querySelector(
			'[data-tab="collect"]',
		);
		if (collectTab) {
	...
	private renderMarketBrowse(body: HTMLElement, info: MarketInfo): void {
		...
		const page = paginateMarketListings(
			listings,
			this.marketBrowsePage,
			MARKET_PAGE_SIZE,
		);
		...
		if (page.pageCount > 1) {
			...
			const pageNumber = formatNumber(page.page + 1, {
				maximumFractionDigits: 0,
			});
			const pageCount = formatNumber(page.pageCount, {
				maximumFractionDigits: 0,
			});
			pager.innerHTML =
			...
			pager
				.querySelectorAll<HTMLButtonElement>("[data-market-page]")
				.forEach((button) => {
					button.addEventListener("click", () => {
						if (button.disabled) return;
						this.marketBrowsePage +=
							button.dataset.marketPage === "next" ? 1 : -1;
						this.lastMarketSig = "";
						audio.click();
						this.renderMarketContent(info);
						body.scrollTop = 0;
	...
	private renderMarketSell(body: HTMLElement, info: MarketInfo): void {
		...
		const suggested = Math.max(
			1,
			item.buyValue ?? Math.max(1, item.sellValue) * 4,
		);
		...
		listBtn.addEventListener("click", () => {
				...
				have > 1
					? Math.max(
							...
							Math.min(
								have,
								parseInt(
									($("#mkt-qty") as HTMLInputElement)?.value || "1",
									10,
								) || 1,
							),
						)
			...
			const gg = Math.max(
				0,
				parseInt(($("#mkt-g") as HTMLInputElement)?.value || "0", 10) || 0,
			);
			const ss = Math.max(
				0,
				parseInt(($("#mkt-s") as HTMLInputElement)?.value || "0", 10) || 0,
			);
			const cc = Math.max(
				0,
				parseInt(($("#mkt-c") as HTMLInputElement)?.value || "0", 10) || 0,
			);
		...
		for (const s of [...sim.inventory]) {
			...
			this.attachTooltip(row, () => {
				...
				if (this.tradeOpen)
					...
					extra = `<div class="tt-sub">${esc(t("itemUi.tooltip.clickUse"))}</div>`;
	...
	private showSellQuantityPrompt(itemId: string, maxCount: number): void {
		...
		const submit = () => {
			const count = Math.max(
				1,
				Math.min(maxCount, Math.floor(Number(input.value) || 0)),
			);
	...
	renderChar(): void {
      ...
      <div class="equip-col equip-col-right" id="equip-col-right"></div>
		...
		const statCell = (stat: StatId) =>
			statCellHtml(this.statModel(stat), STAT_VIEW_DEPS);
      ...
      ${statCell("spi")}${statCell("dodge")}
		...
		el.querySelector('[data-act="share-card"]')?.addEventListener(
			"click",
			() => {
				audio.click();
		...
		for (const cell of el.querySelectorAll<HTMLElement>(
			...
			this.attachTooltip(cell, () =>
				statTooltipHtml(this.statModel(stat), STAT_VIEW_DEPS),
			);
		}
	...
	private renderCharPreview(): void {
		...
		void mechAssets
			...
			.catch((err) =>
				console.error("failed to load mech cosmetic preview:", err),
			);
	}
	...
	private mountCharPreview(
		...
		if (!this.charPreviewCanvas)
			this.charPreviewCanvas = document.createElement("canvas");
		if (!this.charPreview) {
			container.appendChild(this.charPreviewCanvas);
			this.charPreview = new CharacterPreview(
				container,
				this.charPreviewCanvas,
			);
		} else {
	...
	private renderCharSkinPicker(): void {
		...
		const options = characterAppearanceOptions(
			cls,
			this.sim.accountCosmetics.mechChromaIds,
		);
		row.innerHTML = "";
		row.style.setProperty("--class-color", classCss(cls));
		if (options.length <= 1) return;
		if (
			options.some((option) => option.kind === "mech") &&
			!this.mechAssetsPromise
		) {
		...
		for (const option of options) {
			const labelNumber = formatNumber(option.label, {
				maximumFractionDigits: 0,
			});
			...
			b.className =
				"skin-swatch" +
				(option.kind === currentCatalog && option.skin === current
					? " sel"
					: "");
		...
		const currentChroma =
			currentCatalog === "mech" ? MECH_CHROMAS[current] : null;
		if (
			currentChroma &&
			this.sim.accountCosmetics.mechChromaIds.includes(currentChroma.id)
		) {
			const unequip = document.createElement("button");
			unequip.type = "button";
	...
	private skinEventPreviewKey(): string {
		return this.skinEventMode === "mech"
			? "player_mech"
			: `player_${this.sim.cfg.playerClass}`;
	}
	...
	private defaultChoiceSelection(
		rank: SkinRank,
	...
	openSkinEvent(rank: SkinRank, opts?: { mech?: boolean }): void {
		...
		onPortraitsReady(() => {
			if (
				this.skinEventEl?.classList.contains("open") &&
	...
	private renderSkinEvent(): void {
		...
		for (const tierRank of [...SKIN_RANKS].reverse()) {
			...
			const anyAvailable = rankChoices.some((c) =>
				this.skinChoiceAvailable(c.index),
			);
			...
			rankChoices.forEach((ch, i) => {
				...
				if (available) {
					...
					b.setAttribute(
						...
						mech
							? label
							: t("skinEvent.optionAria", { rank: rawName, index: i + 1 }),
					);
		...
		syncSelection();
		(
			swatches.find((b) => b.dataset.choice === this.skinEventSelectedKey) ??
			swatches[0]
		)?.focus();
	}
	...
	private async openPlayerCard(): Promise<void> {
		...
		const walletToggle = back.querySelector<HTMLButtonElement>(
			"[data-wallet-card-toggle]",
		);
		const walletToggleState =
			walletToggle?.querySelector<HTMLElement>(".pc-toggle-state") ?? null;
		...
		const poseButtons = Array.from(
			back.querySelectorAll<HTMLButtonElement>(".pc-pose"),
		);
		...
		const syncWalletToggle = (): void => {
			...
			walletToggle.setAttribute(
				"aria-label",
				t("hudChrome.playerCard.showWalletBadge"),
			);
			walletToggleState.textContent = on
				? t("hud.options.on")
				: t("hud.options.off");
		};
		syncWalletToggle();
		...
		walletToggle?.addEventListener("click", () => {
			...
			this.optionsHooks?.onSettingChange(
		...
		// value anyway.
		this.recomposeOpenCard = () => {
			if (this.cardModalEl === back && metadataReady)
				void compose(requestedPoseIndex);
		};
		...
		[referral, standing] = await Promise.all([
			fetchReferralInfo(),
			fetchStanding(),
		]);
		metadataReady = true;
	...
	private wireCardActions(
					...
					if (
						...
						try {
							await navigator.clipboard.write([
								new ClipboardItem({
									"image/png": cardCanvasToBlob(state.canvas),
								}),
							]);
							copied = true;
						} catch {
		...
		if (typeof nav.canShare === "function") {
			...
			sb.addEventListener("click", async () => {
				...
				try {
					const file = new File(
						[await cardCanvasToBlob(state.canvas)],
						fileName(),
						{ type: "image/png" },
					);
					...
					else if (nav.canShare!({ files: [file] }))
	...
	private cardShareText(data: PlayerCardData): string {
		...
		const tierBit = tier
			? t("playerCard.shareTierBit", { tier: holderTierDisplayName(tier) })
			: "";
	...
	private buildPlayerCardData(
		...
		if (rating !== null)
			combatStats.push({
				label: t("playerCard.arenaStat"),
				value: num(rating),
			});
		if (sim.prestigeRank > 0)
			combatStats.push({
				label: t("game.prestige.rank"),
				value: num(sim.prestigeRank),
			});
		...
		const gear = slots.map((slot) => {
			...
			return {
				...
				color: item
					? (QUALITY_COLOR[item.quality ?? "common"] ?? "#cfc3a0")
					: "#7c7058",
			};
		});
	...
	private talentSummaryHtml(): string {
		...
		if (sp)
			html += `<span>${t("game.talents.role")}: <b>${this.roleLabel(sp.role)}</b></span>`;
		html += `</div>`;
		if (sp)
	...
	private progressionHtml(level: number): string {
		...
		const badges = MILESTONES.filter((m) => unlocked.has(m.id))
			.map(
				(m) =>
					`<span class="ms-badge ms-${m.kind}">${this.milestoneName(m.id)}</span>`,
			)
			.join("");
	...
	private inputDialog(opts: {
		...
		el.innerHTML =
			...
			(opts.copy
				? `<button class="btn" data-copy>${t("game.talents.copy")}</button>`
				: "") +
			(opts.onOk
			...
			`</div>`;
		document.body.appendChild(el);
		const input = el.querySelector(".cd-input") as
			| HTMLInputElement
			| HTMLTextAreaElement;
		const close = () => el.remove();
		const submit = () => {
	...
	private buildDropdown(
		...
		const focusedIndex = () =>
			items.findIndex((it) => it === document.activeElement);
	...
	private setDropdownValue(root: HTMLElement, value: string): void {
		const items = [...root.querySelectorAll<HTMLElement>(".ui-dd-item")];
		const match =
			items.find((x) => x.getAttribute("data-val") === value) ?? null;
	...
	private openChoicePopup(
		...
		pop.setAttribute(
			"aria-label",
			tTalent({ kind: "talentNode", node, field: "name" }),
		);
		...
		const left = Math.max(
			8,
			Math.min(window.innerWidth - pop.offsetWidth - 8, preferredLeft),
		);
		const top = Math.max(
			8,
			Math.min(window.innerHeight - pop.offsetHeight - 8, r.bottom + 12),
		);
		const caretLeft = Math.max(
			14,
			Math.min(pop.offsetWidth - 14, r.left + r.width / 2 - left),
		);
	...
	renderSpellbook(): void {
		...
		for (const abilityId of cls.abilities) {
			...
			const locked = !known;
			const summary = known
				? describeAbilitySummary(known, sim.player.resourceType)
				: "";
			...
			row.setAttribute(
				...
				known
					...
					: t("abilityUi.spellbook.unlearnedAbilityAria", {
							name,
							level: learnLevel,
						}),
			);
	...
	private allocsEqual(a: TalentAllocation, b: TalentAllocation): boolean {
		...
		const ak = Object.keys(a.ranks)
			.filter((k) => a.ranks[k] > 0)
			.sort();
		const bk = Object.keys(b.ranks)
			.filter((k) => b.ranks[k] > 0)
			.sort();
		if (
			ak.length !== bk.length ||
			ak.some((k, i) => k !== bk[i] || a.ranks[k] !== b.ranks[k])
		)
			return false;
		for (const k of ak)
			if ((a.choices[k] ?? null) !== (b.choices[k] ?? null)) return false;
		return true;
	}
	...
	renderTalents(): void {
		...
		const treeSpent = (tree: "class" | "spec") =>
			ct.nodes
				.filter(
					(n) =>
						n.tree === tree && (tree === "class" || n.specId === stage.spec),
				)
		...
		el.querySelectorAll(".tal-tab").forEach((tab) => {
			...
			tab.addEventListener("keydown", (e) =>
				this.keyboardActivate(e as KeyboardEvent, () =>
					switchTab(tab as HTMLElement),
				),
			);
		});
		el.querySelector("[data-close]")?.addEventListener("click", () => {
	...
	private renderSpecTab(
		...
		for (const sp of ct.specs) {
			...
			const specDescription = tTalent({
				kind: "talentSpec",
				spec: sp,
				field: "description",
			});
			const masteryName = tTalent({
				kind: "talentMastery",
				spec: sp,
				field: "name",
			});
			...
			card.setAttribute(
				"aria-label",
				`${specName}, ${this.roleLabel(sp.role)}`,
			);
	...
	private renderTalentTree(
		...
		for (const n of nodes) {
			...
			if (n.kind === "choice" && !cand.choices[n.id])
				cand.choices[n.id] = n.choices![0].id;
			const canAdd =
				ranks < n.maxRank && validateAllocation(cls, cand, total).ok;
			const shape =
				n.kind === "active"
					? "square"
					: n.kind === "choice"
			...
			const chosen =
				n.kind === "choice"
					? n.choices!.find((c) => c.id === stage.choices[n.id])
					: undefined;
			const div = document.createElement("div");
		...
		if (n.requires?.length) {
			const names = n.requires
				.map((r) => {
					...
					return required
						? tTalent({ kind: "talentNode", node: required, field: "name" })
						: r;
				})
				.join(", ");
	...
	private wireTalentFooter(
		...
		const saveStagedBuild = (name: string): void => {
			...
			this.sim.saveLoadout(
				n,
				this.hotbarActions.map((a) =>
					a && a.type === "ability" ? a.id : null,
				),
		...
		const promptNewBuild = (): void => {
			this.inputDialog({
				...
				value: t("hudChrome.talents.defaultBuildName", {
					n: this.sim.loadouts.length + 1,
				}),
		...
		el.querySelector('[data-act="save"]')?.addEventListener("click", () => {
			...
			const active =
				this.sim.activeLoadout >= 0
					? this.sim.loadouts[this.sim.activeLoadout]
					: null;
	...
	private applyLoadoutBar(bar: (string | null)[]): void {
		this.hotbarActions = Array.from(
			{ length: Hud.BAR_ABILITY_SLOTS },
			(_, i) => {
				const v = bar[i];
				return typeof v === "string" && ABILITIES[v]
					? { type: "ability" as const, id: v }
					: null;
			},
		);
		this.saveSlotMap();
	...
	renderQuestLog(): void {
		...
		if (
			...
			!sim.questLog.has(this.selectedQuestLogId)
		) {
		...
		for (const qp of quests) {
			...
			const status =
				qp.state === "ready"
					? t("questUi.log.readyStatus")
					: t("questUi.log.activeStatus");
			const title = questTitle(qp.questId);
			item.type = "button";
			item.className =
				"ql-item" + (qp.questId === this.selectedQuestLogId ? " sel" : "");
			item.setAttribute(
				"aria-pressed",
				qp.questId === this.selectedQuestLogId ? "true" : "false",
			);
			item.setAttribute(
				"aria-label",
		...
		if (this.selectedQuestLogId) {
			...
			const rewardRow = body.querySelector(
				"[data-reward]",
			) as HTMLElement | null;
			if (rewardRow && rewardItem)
				this.attachTooltip(rewardRow, () =>
	...
	private updatePartyFrames(): void {
		...
		const target =
			this.sim.player.targetId !== null
				? this.sim.entities.get(this.sim.player.targetId)
				: null;
		...
		const p = this.sim.player;
		const myGroup =
			info.members.find((m) => m.pid === this.sim.playerId)?.group ?? 1;
		const others = info.members
			.map((m) => ({
				...m,
				oor:
					!m.dead && Math.hypot(m.x - p.pos.x, m.z - p.pos.z) > PARTY_RANGE_YD,
			}))
			.filter(
				(m) =>
					m.pid !== this.sim.playerId && (!info.raid || m.group === myGroup),
			);
		...
		for (const m of others) {
			...
			const resClass =
				m.rtype === "rage" ? "rage" : m.rtype === "energy" ? "energy" : "mana";
			const badge = m.dead
	...
	private openSelfContextMenu(x: number, y: number): void {
		...
		const canConvert =
			!!party &&
			party.leader === this.sim.playerId &&
			!party.raid &&
	...
	openContextMenu(pid: number, name: string, x: number, y: number): void {
		...
		const alreadyGuilded = !!social?.guild?.members.some(
			(m) => m.name === name,
		);
		...
		const entCls =
			ent && ent.kind === "player" ? (ent.templateId as PlayerClass) : null;
	...
	openMarkerMenu(entityId: number, name: string, x: number, y: number): void {
		...
		el.querySelectorAll(".ctx-item").forEach((item) => {
			const activate = () => {
				...
				else if (act && act.startsWith("m"))
					this.sim.setMarker(entityId, Number(act.slice(1)));
			};
	...
	openPetMenu(
		...
		if (dead)
			html += `<div class="ctx-item" data-act="revive">${esc(t("hud.pet.revive"))}</div>`;
		if (!isWarlock)
	...
	private openChatPlayerContextMenu(name: string, x: number, y: number): void {
		...
		const ent =
			livePidForMenu !== null
				? this.sim.entities.get(livePidForMenu)
				: undefined;
		const entCls =
			ent && ent.kind === "player" ? (ent.templateId as PlayerClass) : null;
		...
		el.innerHTML =
			...
			actions
				.map(
					(a) =>
						`<div class="ctx-item" data-act="${a.id}">${esc(a.label)}</div>`,
				)
				.join("");
		hydratePortraits(el);
		this.placePopupAt(el, x, y, 170, 240);
	...
	private saveIgnoredChatNames(): void {
		localStorage.setItem(
			IGNORED_CHAT_NAMES_KEY,
			JSON.stringify([...this.ignoredChatNames]),
		);
	}
	...
	private refreshSocialList(): void {
		const body = $("#social-window").querySelector(
			".soc-body",
		) as HTMLElement | null;
		if (!body) return;
		const online = this.sim.socialInfo !== null;
	...
	private guildHtml(): string {
		...
		if (!guild)
			return `<div class="soc-empty">${esc(t("hud.social.noGuild"))}</div>`;
		const me = guild.rank;
		...
		const rows = guild.members
			.map((m) => {
				...
				const canKick =
					!self &&
					((me === "leader" && m.rank !== "leader") ||
						(me === "officer" && m.rank === "member"));
				if (canKick)
		...
		const groupHtml = (group: 1 | 2): string => {
			const rows =
				groups[group]
					...
					.join("") ||
				`<div class="soc-empty">${esc(t("hud.social.raidGroupEmpty"))}</div>`;
	...
	private addRow(
		...
		return (
			`<div class="soc-add">` +
			(suggest
				? `<div class="soc-suggest" data-for="${field}" role="listbox"></div>`
				: "") +
	...
	private wireSocialChrome(el: HTMLElement): void {
		...
		el.querySelectorAll(".soc-tab").forEach((t) => {
			t.addEventListener("click", () => {
				this.socialTab = (t as HTMLElement).dataset.tab as
					| "friends"
					| "guild"
					| "ignore"
					| "raid";
		...
		const field = (sel: string): string =>
			(
				el.querySelector(
					`input[data-field="${sel}"]`,
				) as HTMLInputElement | null
			)?.value.trim() ?? "";
		const submit = (act: string | undefined): void => {
			if (act === "friend-add")
				void this.socialResolveAndAct("friend", field("friend"));
			else if (act === "block-add")
				void this.socialResolveAndAct("ignore", field("ignore"));
			else if (act === "guild-invite")
				void this.socialResolveAndAct("ginvite", field("ginvite"));
		...
		el.querySelectorAll(".soc-add input:not([data-suggest])").forEach((inp) => {
			inp.addEventListener("keydown", (e) => {
				...
				submit(
					(inp.parentElement?.querySelector(".btn") as HTMLElement | null)
						?.dataset.act,
				);
			});
		});
	...
	private wireSuggest(el: HTMLElement): void {
		el.querySelectorAll("input[data-suggest]").forEach((node) => {
			...
			input.addEventListener("keydown", (e) => {
				const ke = e as KeyboardEvent;
				const open =
					this.socialSuggest.field === field &&
					this.socialSuggest.items.length > 0;
	...
	private renderSuggest(
		...
		box.querySelectorAll(".soc-sugg-item").forEach((it) => {
			it.addEventListener("mousedown", (e) => {
				e.preventDefault();
				void this.socialResolveAndAct(
					kind,
					(it as HTMLElement).dataset.name ?? "",
				);
			});
	...
	private highlightSuggest(field: string): void {
		...
		box.querySelectorAll(".soc-sugg-item").forEach((it) => {
			const on =
				Number((it as HTMLElement).dataset.i) === this.socialSuggest.index;
	...
	private async socialResolveAndAct(
		...
		if (kind === "friend") {
			...
			this.setSocialNotice(
				t("hud.social.friendAdded", { name: exact.name }),
				false,
			);
			...
			this.setSocialNotice(
				t("hud.social.nowIgnoring", { name: exact.name }),
				false,
			);
			...
			this.setSocialNotice(
				t("hud.social.guildInvited", { name: exact.name }),
				false,
			);
	...
	addItemToTrade(itemId: string): void {
		...
		const have =
			this.sim.inventory.find((s) => s.itemId === itemId)?.count ?? 0;
		if (existing) {
	...
	private updateTradeWindow(): void {
      ...
      <div class="trade-hint">${esc(t("hud.trade.hint"))}</div>`;
		...
		acceptBtn.textContent = info.myAccepted
			? t("hud.trade.waiting")
			: t("hud.trade.accept");
		...
		el.querySelectorAll(".trade-item.mine").forEach((row) => {
			row.addEventListener("click", () => {
				const itemId = (row as HTMLElement).dataset.item!;
				const idx = this.stagedTrade.items.findIndex(
					(s) => s.itemId === itemId,
				);
				if (idx >= 0) {
					this.stagedTrade.items[idx].count--;
					if (this.stagedTrade.items[idx].count <= 0)
						this.stagedTrade.items.splice(idx, 1);
					this.pushTradeOffer();
				}
			});
		});
	...
	private renderOptions(): void {
		...
		if (this.optionsView !== "keybinds")
			$("#options-menu").classList.remove("kb-wide");
		if (this.optionsView !== "performance")
			$("#options-menu").classList.remove("perf-wide");
		// The overlay is draggable only while the Performance sub-view is open.
		this.optionsHooks?.perfOverlay.setPlacement(
			this.optionsView === "performance",
		);
		if (this.optionsView === "keybinds") {
			this.renderKeybinds();
			return;
		...
		if (this.bugReportHooks)
			add(t("hudChrome.bugReport.menuButton"), () => goto("bugreport"));
	...
	private settingBoolToggle(
		...
		toggle.addEventListener("click", () => {
			...
			hooks.onSettingChange(
				key,
				hooks.settings.set(key, !hooks.settings.get(key)),
			);
			sync();
		});
		row.append(name, toggle);
	...
	private settingChoice(
		...
		const sync = () => {
			...
			for (const btn of [
				...wrap.querySelectorAll<HTMLButtonElement>("button[data-value]"),
			]) {
	...
	private renderBugReport(): void {
		...
		desc.setAttribute(
			"placeholder",
			t("hudChrome.bugReport.descriptionPlaceholder"),
		);
		...
		if (shot) {
			...
			toggle.className = "btn set-toggle";
			const syncToggle = () => {
				toggle.textContent = includeShot
					? t("hud.options.on")
					: t("hud.options.off");
				...
				toggle.setAttribute(
					"aria-label",
					t("hudChrome.bugReport.includeScreenshot"),
				);
	...
	private renderGraphics(): void {
		...
		if (
			Math.round(this.optionsHooks?.settings.get("graphicsPreset") ?? 0) === 5
		) {
			this.settingChoice(
				body,
				t("hud.options.terrainDetail"),
				"terrainDetail",
				[
		...
		this.settingChoice(
			body,
			t("hudChrome.options.browserEffects"),
			"browserEffects",
			[
		...
		this.settingToggle(
			body,
			t("game.settings.showOverflowXp"),
			"showOverflowXp",
		);
		...
		if (useTouchInterface())
			this.settingSlider(
				body,
				t("hud.options.joystickDeadzone"),
				"joystickDeadzone",
			);
	...
	private renderAudio(): void {
		...
		toggle.className = "btn set-toggle";
		const sync = () => {
			toggle.textContent = music.enabled
				? t("hud.options.on")
				: t("hud.options.off");
		...
		this.settingBoolToggle(
			body,
			t("hudChrome.options.footstepSounds"),
			"footstepSfx",
		);
		this.settingBoolToggle(
			body,
			t("hudChrome.options.clickFeedback"),
			"clickFeedback",
		);
		this.settingsViewFooter();
	}
	...
	private renderInterface(): void {
		...
		this.settingBoolToggle(
			body,
			t("hud.options.frostedPanels"),
			"frostedPanels",
		);
		this.settingBoolToggle(
			body,
			t("hud.options.highContrastText"),
			"highContrastText",
		);
		...
		const sync = () => {
			tsToggle.textContent = this.chatTimestamps
				? t("hud.options.on")
				: t("hud.options.off");
	...
	private renderPerformance(): void {
		const hooks = this.optionsHooks;
		if (!hooks) return;
		this.perfSettings ??= new PerfOverlaySettingsPanel(
			this.perfSettingsHost(hooks),
		);
	...
	private actionDisplayName(actionId: string, fallback: string): string {
		if (!actionId.startsWith("slot"))
			return BIND_ACTION_LABEL_KEYS[actionId]
				? t(BIND_ACTION_LABEL_KEYS[actionId])
				: fallback;
	...
	private clickMoveMouseButtonRow(parent: HTMLElement): void {
		...
		toggle.addEventListener("click", () => {
			audio.click();
			const next =
				normalizeClickMoveButton(hooks.settings.get("clickToMoveButton")) === 0
					? 2
					: 0;
	...
	private renderController(): void {
		...
		this.settingBoolToggle(
			body,
			t("hudChrome.controller.enable"),
			"gamepadEnabled",
		);
		this.settingBoolToggle(
			body,
			t("hudChrome.controller.invertY"),
			"gamepadInvertY",
		);
		this.settingSlider(
			body,
			t("hudChrome.controller.deadzone"),
			"gamepadStickDeadzone",
		);
		this.settingSlider(
			body,
			t("hudChrome.controller.cameraSpeed"),
			"gamepadCameraSpeed",
			{ fmt: (v) => formatNumber(v, { maximumFractionDigits: 1 }) },
		);
		this.settingSlider(
			body,
			t("hudChrome.controller.vibration"),
			"gamepadVibration",
		);
		...
		if (hooks) {
			...
			for (const { button, action } of hooks.gamepad.entries()) {
				...
				const dd = this.buildDropdown(opts, action, (v) =>
	...
	private renderKeybinds(): void {
		...
		this.settingToggleKeybind(
			el,
			t("hud.keybinds.actions.attackMove"),
			"attackMove",
		);
		this.settingToggleKeybind(
			el,
			t("hud.options.leftHandedTouch"),
			"leftHandedTouch",
		);
		this.settingToggleKeybind(
			el,
			t("hud.options.filterProfanity"),
			"filterProfanity",
		);
		const note = document.createElement("div");
		note.className = "kb-note";
		note.textContent =
			this.keybindNote || t("hud.options.keybindHelpMouseCamera");
		el.appendChild(note);
		const cols = document.createElement("div");
		...
		for (const category of BIND_CATEGORIES) {
			const visible = BIND_ACTIONS.filter(
				(a) =>
					a.category === category && (a.id !== "attackMove" || attackMoveOn),
			);
			...
			for (const action of visible) {
				...
				for (let index = 0; index < 2; index++) {
					const capturing =
						this.capturingKey?.action === action.id &&
						this.capturingKey?.index === index;
					const key = document.createElement("button");
					key.className = "btn kb-key" + (capturing ? " capturing" : "");
					key.textContent = capturing
						? "..."
						: this.keybinds.labelAt(action.id, index) ||
							t("hud.options.unbound");
					key.title =
						index === 0 ? t("hud.options.primary") : t("hud.options.alternate");
					key.setAttribute(
						"aria-label",
						`${this.actionDisplayName(action.id, action.label)} ${key.title}`,
					);
					key.addEventListener("click", () =>
						this.beginCapture(action.id, index, action.label),
					);
					row.appendChild(key);
				}
	...
	private beginCapture(
		...
		this.optionsHooks.captureKey((code) => {
			...
			if (code === null) {
				...
				this.keybindNote = t("hud.options.keybindReserved", {
					key: keyLabel(code),
				});
			}
...
function describeAbilitySummary(
	...
	if (known.def.cooldown > 0) {
		parts.push(
			t("abilityUi.tooltip.cooldownSeconds", {
				seconds: formatAbilityNumber(known.def.cooldown),
			}),
		);
	}
	return parts.join(" · ");
...
function abilityCastLine(known: ResolvedAbility): string {
	...
	if (known.castTime > 0) {
		return t("abilityUi.tooltip.castSeconds", {
			seconds: formatAbilityNumber(known.castTime),
		});
	}
...
function abilityRequirementLines(def: AbilityDef): string[] {
	...
	if (def.requiresOutOfCombat)
		lines.push(t("abilityUi.tooltip.requiresOutOfCombat"));
	...
	if (def.targetType === "friendly")
		lines.push(t("abilityUi.tooltip.friendlyTarget"));
...
function dotTitle(
	...
	return zone
		? t("hud.social.statusWithZone", {
				status: label,
				zone: localizeZone(zone),
			})
		: label;
}
