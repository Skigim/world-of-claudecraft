...
const skeletonClips = (
	attack: string[],
	flourish = "Skeletons_Awaken_Standing",
): ClipMap => ({
...
function mechChromaUrl(c: MechChroma): string {
	if (c.rank === "uncommon")
		return `${MECH_DIR}/uncommon/combatmech_${c.id}.png`;
...
function mechEmissiveUrl(c: MechChroma): string | null {
	return c.rank === "epic"
		? `${MECH_DIR}/epics/combatmech_epic_${c.id}_emis.png`
		: null;
}
...
export function visualKeyFor(e: Entity): string {
	if (e.kind === "player") {
		...
		return VISUALS[`player_${e.templateId}`]
			? `player_${e.templateId}`
			: "player_warrior";
	}
	if (e.kind === "mob") {
...
export function visualAssetUrlForGraphics(
	url: string,
	...
	return [
		...new Set(
			manifestUrls().map((url) =>
				visualAssetUrlForGraphics(url, standardMaterials),
			),
		),
	];
}
